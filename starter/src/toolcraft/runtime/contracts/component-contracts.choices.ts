...
export const TOOLCRAFT_CHOICE_COMPONENT_CONTRACTS = {
  ...
  panelActions: {
    ...
    aiUsageRules: [
      ...
      'Runtime export button labels follow the current format: Export PNG/JPG from export.image.format, Export MP4/WebM from export.video.format, and fixed Export SVG. Update on live edits, restored/imported settings, Undo/Redo and reset; keep action roles, values, button identity and export dispatch unchanged. Product code must not rename or replace runtime export actions.',

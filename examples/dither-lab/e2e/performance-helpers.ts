...
export async function dragToolcraftSliderByLabel(
  ...
  await expect(sliderSurface, `Toolcraft slider surface "${label}" should be visible`).toBeVisible();
  ...
  const currentRatio = await slider.evaluate((element) => {
    ...
    const value = Number(
      htmlElement.getAttribute("aria-valuenow") ??
        (htmlElement as HTMLInputElement).value ??
        "0",
    );
    const denominator = max - min;

    return denominator === 0 || !Number.isFinite(value)
      ? 0
      : (value - min) / denominator;
  });
  const safeCurrentRatio = Math.min(0.98, Math.max(0.02, currentRatio));
  const safeTargetRatio = Math.min(0.98, Math.max(0.02, targetRatio));
  const startX = box.x + box.width * safeCurrentRatio;
  const endX = box.x + box.width * safeTargetRatio;
  ...
  if (targetRatio <= 0 || targetRatio >= 1) {
    await slider.focus();
    await page.keyboard.press(targetRatio <= 0 ? "Home" : "End");
  }
}

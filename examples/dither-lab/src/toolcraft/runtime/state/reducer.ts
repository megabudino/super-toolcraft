...
function getResetMediaPatch(
  ...
  if (
    ...
    mediaAssets.every((asset, index) =>
      JSON.stringify(asset) === JSON.stringify(state.mediaAssets[index]),
    ) &&
    layers.length === state.layers.length &&
    layers.every((layer, index) => JSON.stringify(layer) === JSON.stringify(state.layers[index])) &&

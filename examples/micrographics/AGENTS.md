...
- Before delivery, replace the starter worklog with `Mode: product`, add one `Decision Trail` entry per coherent user-visible delivery batch, and record concrete decisions for renderer, timeline, layers, controls, export, and performance. Each entry names the user-visible result, source/reference checked, contract rules applied, rejected alternatives, state/output mapping, and focused development checks. Worklog prose records context and planned or attempted commands; only the protected source-bound receipt proves a passed delivery. `npm run test` fails if the worklog is missing, lacks the decision trail, or still describes the neutral starter.
...
First product delivery runs full functional acceptance plus a bounded production-build prototype smoke. It does not run or claim the complete performance matrix and does not require a durable baseline. Ordinary later delivery uses exact targeted checks only and cannot silently run full performance. A direct or repeated performance complaint starts exactly one `performance-iteration`: record one canonical command plus one identical executed `Run`, bind the protected measurements to the exact Request evidence, run exact affected functional and canonical performance tests against the reachable development fixture, deliver the verified app, then stop and wait for user evaluation. The same request authority cannot produce a second successful pass; a new complaint starts another bounded iteration and records deltas only when the previous measurements are compatible. Complaint wording, repetition, tier, filename, and touched subsystem never launch the complete matrix automatically. After two consecutive compatible iterations, offer the user the slower complete audit if performance remains unacceptable. An explicit natural-language request for that audit or explicit acceptance of the offer authorizes the agent acting as operator to run `npm run verify:perf`; the user does not need to know the command name. Detailed classification belongs only to the performance docs.
...
The app is not complete until `npm run verify:delivery` produces the lifecycle-appropriate protected receipt. A prototype receipt proves full functionality and bounded responsiveness, not full performance certification. Agent-controlled browser checks remain useful for targeted diagnosis and visual verification, but terminal prose cannot mint or upgrade a receipt. After the first delivery, exact `--unit-test`, `--browser-test`, and `--performance-test` selectors go to `verify:delivery`; the impact inventory derives the minimum tier and affected passes from the immediately previous successful delivery. A bare tier, prose claim, or product-authored report is invalid.
...
The protected runner selects prototype functional-plus-smoke proof, ordinary targeted proof, or one targeted `performance-iteration` from its receipts. Do not run `verify:quick`, `verify:perf:record-iteration`, or `verify:final` before or after a successful `verify:delivery` for unchanged source. `npm run verify:perf` is the separate operator/CI full-certification command; run it only after an explicit user request or accepted agent offer, never as an inferred continuation of a complaint.

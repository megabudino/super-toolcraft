...
`src/app/app-performance-impact.json` maps every current product production module to `functional` or `performance` impact. A performance entry is reserved for a module that can change a named pass's execution, invalidation, workload, resource lifecycle, or measured output. Pure wiring, schema declarations, labels, acceptance metadata, and orchestration are functional unless they change one of those semantics. Performance entries name only the exact renderer pass ids they can change, every declared pass has at least one real implementation owner, and the inventory contains no stale or missing module paths. Blanket ownership where every module claims every pass is invalid. This mapping is the authority for verification scope after prototype delivery.
...
During implementation, run targeted unit and functional browser checks for feedback without minting delivery evidence. At the coherent delivery boundary, the protected delivery runner diffs current sources against the immediately previous successful delivery, resolves changed modules through `app-performance-impact.json`, and derives the minimum tier. Functional modules require a passed functional browser test. Performance modules require exact tests whose protected report covers the expected pass ids and canonical path ids. A durable full-performance baseline, when one exists, remains historical evidence and is not reused as the ordinary-delivery diff anchor. The report is bound to the selected test names, nonce, and current source hash, so a bare tier or product-authored JSON is not evidence.
...
The conversational lifecycle has three protected modes:

1. **Prototype delivery.** The first product delivery runs full functional acceptance plus bounded production-build prototype smoke. Smoke uses a fixed two-times tolerance over the selected canonical path profile, catching material stalls without treating browser scheduling noise as certification failure. The receipt records `prototype-smoke`, requires no durable baseline, writes no full-performance report, and cannot claim profile compliance or the complete matrix.
2. **Ordinary product delivery.** Later ordinary work runs exact targeted checks only. It preserves a baseline when present and cannot silently run full performance or create one.
3. **Performance iteration.** A direct or repeated performance complaint starts exactly one `performance-iteration`. The latest Decision Trail records one canonical command and Verification records one identical executed `Run`; the protected report binds them to the exact Request evidence, so one complaint cannot silently authorize a second pass. Run exact affected functional checks and canonical performance tests through `npm run verify:delivery -- --reason=performance-iteration --tier=<3-or-4> --performance-test="browser perf: exact title"`. The selected tests use the reachable development fixture and store independently validated `cold`, `warm`, and `sustained` measurements. A compatible immediately previous report produces diagnostic metric deltas; an incompatible or absent report produces an explicit non-comparable result. Current absolute budgets remain the pass/fail authority. Deliver the verified app, then stop and wait for user evaluation. A later complaint, even with repeated wording, must be recorded as a new Decision Trail iteration and starts another bounded iteration.
...
Use `npm run verify:delivery` once for the coherent delivery batch. Complaint wording, repetition, tier, filename, and touched subsystem never launch the complete matrix automatically. A targeted failure triggers targeted diagnosis, not an automatic full-suite run, and a passing iteration stops after returning the verified app for user evaluation. Do not compose `verify:quick`, performance commands, and `verify:final` into another delivery chain.
...
`npm run verify:perf` is the separate protected operator/CI full-certification command. It runs maximum fixtures for the complete matrix, records `full-performance`, and creates or refreshes the durable baseline. An explicit natural-language request for a complete audit or explicit acceptance of the agent's offer authorizes the agent acting as operator to run it; the user does not need to know the command name. A complaint, repeated complaint, worklog iteration evidence, tier, filename, or changed subsystem alone never authorizes it. Treat the audit as diagnosis and certification: fix and rerun failed paths with focused checks, then run the complete matrix only once at the requested certification boundary.

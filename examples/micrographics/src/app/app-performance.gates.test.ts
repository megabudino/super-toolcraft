...
describe("Toolcraft starter performance gates", () => {
  it("publishes separate browser acceptance and performance fallback gates", () => {
    ...
    expect(deliveryLifecycle).toContain("runToolcraftAggregateFunctionalGate");
    expect(deliveryLifecycle).toContain("executeToolcraftTargetedVerification");
    ...
    const targetedExecution = readFileSync(
      join(projectDir, "scripts/toolcraft-targeted-verification-execution.mjs"),
      "utf8",
    );
  ...
  it("keeps verification lifecycle authority in the runtime contract", () => {
    ...
    expect(TOOLCRAFT_PERFORMANCE_VERIFICATION_LIFECYCLE).toEqual({
      ...
      fullAudit: {
        ...
        command: "verify:perf",
        executionAuthority:
          "explicit-user-request-or-accepted-agent-offer",
        recommendation:

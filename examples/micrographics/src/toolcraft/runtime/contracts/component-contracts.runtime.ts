...
export const TOOLCRAFT_RUNTIME_COMPONENT_CONTRACTS = {
  ...
  performanceAcceptance: {
    aiUsageRules: [
      ...
      "Prototype smoke requires an exact reachable bounded development fixture at normalized pressure 0.8 and fails with a configuration error when it is unavailable; prototype smoke never falls back to maximum. Ordinary targeted performance checks keep the existing maximum fallback only when no exact reachable development vector exists. Explicit performance checkpoints run the complete current matrix with maximum fixtures.",
  ...
  controlLabels: {
    aiUsageRules: [
      ...
      "Generated product apps must export appControlSectionInventory beside acceptance. Every product controls section declares title, exact targets, groupingReason, and entity or workflowStage; intentional splits of one target entity require workflowStage and splitReason on every split section.",

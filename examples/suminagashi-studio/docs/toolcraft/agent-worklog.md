...
- User-visible result: The `suminagashi-studio` template is admitted as a complete standalone snapshot; original framework hashes remain authoritative. Canonical identity regeneration, where needed, uses the upstream identity generator.
...
- User-visible result: The production build and Vercel deployment now serve the app at `/demos/suminagashi-studio/`, including nested client routes and bundled assets.
...
- Verification: the exhaustive four-test gateway contract, production build, canonical root browser load, and `/demos/suminagashi-studio/review-deep-link` reload passed with `200 text/html` and all asset responses successful. The pre-existing copied-runtime integrity blocker remains unchanged.
...
- Source and contract: `apps/website/public/social-previews/og-toolcraft-v2.jpg` remains the single asset source. The metadata uses its absolute `https://toolcraft.sh/` URL so neither the `/demos/suminagashi-studio/` Vite base nor a direct Vercel hostname can rewrite it incorrectly.
...
- User-visible result: Renamed the standalone product to `Suminagashi Studio` and aligned its repository package plus public demo base to `suminagashi-studio`. Product rendering, controls, defaults, and export behavior remain unchanged.
...
- State/output mapping: Package name, HTML title, control/acceptance identity, persistence/settings-transfer namespace where present, Vite base, public asset prefix, and Vercel rewrites now use `suminagashi-studio`. Changed persistence namespaces intentionally reset prior browser-local settings.

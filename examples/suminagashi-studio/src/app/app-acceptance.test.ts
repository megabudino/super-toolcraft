...
import {
  ...
  appProductReadiness,
  appTransferMode,
  validateToolcraftAcceptanceCoverage as validateToolcraftAcceptanceCoverageForApp,
} from "./app-acceptance";
...
const neutralTransferMode: ToolcraftTransferMode = {
  animationIntent: { mode: "none" },
  mode: "new-toolcraft-app",
};
const appAcceptanceEntries = new WeakSet<object>(appAcceptance as readonly object[]);
...
function validateToolcraftAcceptanceCoverage(
  schema: ResolvedToolcraftAppSchema = appSchema,
  acceptance: readonly ToolcraftComponentAcceptance[] = appAcceptance,
  transferMode: ToolcraftTransferMode = schema === appSchema ? appTransferMode : neutralTransferMode,
): string[] {
  const acceptanceForSchema =
    schema === appSchema
      ? acceptance
      : acceptance.filter((entry) => !appAcceptanceEntries.has(entry));

  return validateToolcraftAcceptanceCoverageForApp(
    schema,
    acceptanceForSchema,
    transferMode,
  );
  ...
  return {
    controls: {
      includeBackground: {
        defaultValue: true,
        description:
          "Controls preview and PNG background visibility.",
        label: "Include",
...
describe("Toolcraft template app acceptance coverage", () => {
  ...
  it("declares the expected Toolcraft assembly mode", () => {
    expect(appTransferMode).toMatchObject({
      animationIntent: { mode: "autonomous" },
  ...
  it("requires product output apps to expose export actions in the sticky footer", () => {
    ...
    expect(
      ...
      "Live preview must read include-background state through shouldIncludeToolcraftPreviewBackground(state) so turning Include off hides the product preview background.",
    ...
    const hasVideoExportAction = panelActionTexts.some(textLooksLikeVideoExport);

    if (!hasVideoExportAction) {
      expect(
        panelActionTexts,
        "Image-only product policy must not expose Export Video through panelActions.",
      ).not.toEqual(expect.arrayContaining([expect.stringMatching(/\bVideo\b/i)]));
      expect(
        productImplementationSource,
        "Image-only product policy must not retain MediaRecorder/captureStream video export code.",
      ).not.toMatch(/\bMediaRecorder\b|\bcaptureStream\b|\bexport\.video\b|\bexport-video\b/);
      return;
    }
  ...
  it("accepts png export apps that wire background controls into the schema", () => {
    ...
    const errors = validateToolcraftAcceptanceCoverage(schemaWithBackgroundControls, [
      makeControlAcceptance("appearance.background", "color"),
      {
        ...makeControlAcceptance("export.includeBackground", "switch"),
        expectedObservable:
          "Turning Include off makes PNG output transparent and hides the live preview product background.",
        userAction:
          "Toggle Include off, verify preview has no product background, and export PNG with alpha.",
      },
      {
        actionCoverage: ["export.png"],
        automated: true,
  ...
  it("requires include-background acceptance to hide preview background and png background", () => {
    const schemaWithBackgroundControls = defineToolcraft({
      canvas: {
        enabled: true,
        sizing: { mode: "editable-output" },
    ...
    expect(
      ...
      expect.arrayContaining([
        expect.stringContaining(
          "controls background inclusion and acceptance must prove disabling it makes PNG output transparent and hides the live preview product background.",
        ),
      ]),
    );
  });

...
test("browser performance tests use real Toolcraft interactions", () => {
  ...
  for (const scenario of appPerformance.scenarios) {
    ...
    if (scenario.interaction === "animation-viewport-drag") {
      ...
      ).toMatch(/measureToolcraftInteraction\s*\(|dragToolcraftCanvasViewport\s*\(/);
      expect(
        browserTestSource,
        `${scenario.id} must use the Toolcraft viewport drag helper.`,
      ).toMatch(/dragToolcraftCanvasViewport\s*\(/);
      expect(
        browserTestSource,
    ...
    if (scenario.interaction === "viewport-zoom-stress") {
      ...
      ).toMatch(/measureToolcraftInteraction\s*\(|zoomToolcraftCanvasViewport\s*\(/);
      expect(
        browserTestSource,
        `${scenario.id} must use the Toolcraft toolbar zoom helper.`,
      ).toMatch(/zoomToolcraftCanvasViewport\s*\(/);
      expect(
        browserTestSource,

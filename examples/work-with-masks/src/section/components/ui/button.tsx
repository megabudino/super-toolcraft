  ...
  ),
  {
    variants: {
      ...
      size: {
        ...
        sm: 'h-9 b-rounded-sm px-4 text-base',
        md: 'h-10 b-rounded-md px-4 text-[0.8125rem]',
        header: 'h-10 rounded-[0.89rem] px-5 text-base leading-none font-semibold',
        hero: 'h-14 rounded-[1.1rem] px-7 text-base leading-none font-semibold',
        cta: cn(
          'h-16 rounded-[5.625rem] px-7 leading-none',
          'text-(length:--text-body-lg) tracking-(--text-body-lg--letter-spacing)',
        ),
...
function Button({ className, variant, size, asChild = false, children, ...rest }: ButtonProps) {
  ...
  if (asChild && React.isValidElement(children)) {
    ...
    return React.cloneElement(elementToClone, {
      ...
      children: <span className={sectionClasses(baseTextClasses)}>{elementToClone.props.children}</span>,
    });
  }

  return (
    <button className={sectionClasses(classes)} data-slot="button" {...rest}>
      <span className={sectionClasses(baseTextClasses)}>{children}</span>
    </button>
  );
}

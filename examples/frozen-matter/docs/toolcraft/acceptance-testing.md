...
`npm run verify:final` must pass before final delivery. Incremental edits use the minimum tier and affected passes derived from `app-performance-impact.json`. The first stable working version runs `npm run verify:perf`; explicit performance work after that baseline runs `npm run verify:perf:refresh`. Other later passes use `npm run verify:perf:record-iteration -- --tier=<tier>` with exact unit/browser/performance test arguments. The protected runner executes the named checks and binds performance evidence to expected pass ids and canonical path ids before recording current-source coverage, so prose, a bare tier, or a product-authored report cannot mint evidence. Agent-controlled browser checks remain useful for diagnosis but cannot replace protected receipts.

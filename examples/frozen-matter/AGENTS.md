...
Feature loops after the first working version do not run the full performance suite by default. Renderer, canvas, animation, export, timeline, layers, `canvas.renderScale`, bug fixes, and performance-sensitive controls still need targeted functional/browser checks first, plus targeted performance scenarios only when they directly exercise the touched workload/viewport/export path. Filename, verification tier, and touched subsystem do not trigger the full suite by themselves. Record the targeted checks and why the full checkpoint was not required in the verification note or worklog.

The first stable working product app version is not complete until `npm run verify:perf` has produced the durable full-checkpoint baseline plus current-source receipt and `npm run verify:final` has passed against them. Agent-controlled browser checks remain useful for targeted diagnosis and visual verification, but terminal prose cannot mint the baseline. Worklog prose is context, not execution proof. After the first checkpoint, use `npm run verify:perf:record-iteration -- --tier=<0|1|2|3|4> ...`; the impact inventory derives the minimum tier and affected passes, while the protected runner executes exact tests and accepts performance proof only when the reporter covers those passes and canonical paths. A bare tier, prose claim, or product-authored report is invalid.
...
For the first stable working product delivery, run `npm run verify:perf` once before `npm run verify:final`; its protected Playwright runner records the durable baseline and current checkpoint. Later delivery passes use the protected targeted runner, which executes their tier-specific checks before recording a current receipt. The final gate rejects a missing baseline or missing, mismatched, or stale current receipt.

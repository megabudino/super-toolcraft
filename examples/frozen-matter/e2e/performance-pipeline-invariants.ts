...
export function getPipelinePhaseContinuityErrors({
  ...
  const errors: string[] = [];
  if (!snapshotsMatch(cold.after, warm.before)) {
    errors.push(
      `Path "${path.id}" must preserve exact pipeline counters from cold.after to warm.before.`,
    );
  }
  if (!snapshotsMatch(warm.after, sustained.before)) {
    errors.push(
      `Path "${path.id}" must preserve exact pipeline counters from warm.after to sustained.before.`,
    );
  }
  return errors;
}

...
function evaluateTargetedSelection({
  canonicalPaths,
  expectedPaths,
  selectedTests,
  target,
  ...
  if (
    requestedPassIds.length === 0 ||
    requestedPassIds.length !== target.passIds.length
  ) {
    errors.push(
      "Targeted performance reporting requires non-empty unique renderer pass ids.",
    );
  }
  ...
  const paths = canonicalPaths.filter((path) =>
    path.invalidates.some((passId) => requestedPassIds.includes(passId)),
  );
...
export function evaluateToolcraftBrowserPerformanceRun({
  ...
  if (targetedPerformanceReport) {
    const targeted = evaluateTargetedSelection({
      canonicalPaths,
      expectedPaths,
      selectedTests,
  ...
  return { checkpoint, errors, performance: performanceReport, targetedPaths };
}

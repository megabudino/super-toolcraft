...
import { getToolcraftControlRendererKind } from "../controls-panel/renderers/controls-panel-renderer-registry";
    ...
    () => [
      ...new Set(
        (store.getCommittedState().schema.panels.controls?.sections ?? [])
          ...
          .filter(
            (controlType): controlType is RegisteredCanvasHandleType =>
              getToolcraftControlRendererKind(controlType) ===
                "canvas-handle" &&
              isRegisteredCanvasHandleType(controlType),
          ),
      ),
    ],
    [store],

...
Before implementation, classify the coherent delivery batch once and record the planned checks:
...
Use `npm run verify:ui` only as a focused development check when browser acceptance is needed without performance. Run `npm run verify:delivery` once for each coherent user-visible delivery batch. First product delivery runs full functional acceptance plus bounded production-build prototype smoke; ordinary later delivery runs exact targeted checks relative to the prior delivery; a performance complaint runs one bounded `performance-iteration` with exact affected selectors and then returns the app for user evaluation. Full `npm run verify:perf` is a separate operator/CI certification command. Classification details remain canonical in the performance docs.

...
test("browser smoke: prototype smoke keeps a bounded two-times profile tolerance", () => {
  const budget = getToolcraftPrototypeSmokePathBudget(interactionPath, [
    interactionScenario(interactionPath),
  ]);

  expect(budget).toEqual({
    maxFrameGapMs: 160,
    maxInteractionMs: 1_000,
    maxLongTaskMs: 100,
  });
});

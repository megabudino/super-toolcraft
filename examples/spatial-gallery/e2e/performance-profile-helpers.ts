...
export function getToolcraftPrototypeSmokePathBudget(
  path: ToolcraftPerformancePath,
  pathScenarios: readonly ToolcraftPerformanceScenario[] = [],
): ToolcraftPerformancePathBudget {
  const budget = getToolcraftPerformancePathBudget(path, pathScenarios);
  return Object.freeze(
    Object.fromEntries(
      Object.entries(budget).map(([key, value]) => [key, value * 2]),
    ) as ToolcraftPerformancePathBudget,
  );
}

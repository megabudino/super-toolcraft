...
test("rejects blanket all-module ownership of every renderer pass", () => {
  const result = validateToolcraftPerformanceImpactInventory(
    {
      modules: [
        {
  ...
  assert.match(
    result.errors.join("\n"),
    /every product production module claims every renderer pass/iu,
  );
});

test("allows shared performance owners when ownership is meaningfully narrower", () => {
  const result = validateToolcraftPerformanceImpactInventory(
    {
      modules: [
        {

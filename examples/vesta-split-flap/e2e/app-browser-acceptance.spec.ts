...
import {
  getRequiredCreativeAppsKitControlPartCoverage,
  appAcceptance,
  appTransferMode,
} from "../src/app/app-acceptance";
import { appSchema } from "../src/app/app-schema";
import { expectNoForbiddenCanvasUi } from "./canvas-handle-helpers";
import { expectCreativeAppsKitSegmentedControlCellsPreservePadding } from "./performance-helpers";
...
function getCompoundPartControls() {
  return (appSchema.panels.controls?.sections ?? []).flatMap((section) =>
    Object.values(section.controls).filter(
      (control) => getRequiredCreativeAppsKitControlPartCoverage(control).length > 0,
    ),
  );
}
...
test("browser renders the Creative Apps Kit template shell instead of a reference iframe shell", async ({
  page,
}) => {
  await page.goto("/");

  await expect(page.locator('[data-slot="creative-apps-kit-runtime-app"]')).toBeVisible();
  ...
  const nonCanvasIframeCount = await page.evaluate(
    () =>
      Array.from(document.querySelectorAll("iframe")).filter(
        (frame) => !frame.closest("[data-creative-apps-kit-canvas-slot]"),
      ).length,
  );

  expect(
    nonCanvasIframeCount,
    "Reference iframes may not replace the Creative Apps Kit shell. Preserve reference output inside CreativeAppsKitApp canvasContent.",
  ).toBe(0);
});

test("browser preserves the Creative Apps Kit canvas backing surface", async ({ page }) => {
  if (!appSchema.assembly.surfaces.canvas.enabled) {
    return;
  }
...
test("canvas no-UI helper rejects unclassified canvas text", async ({ page }) => {
  await page.setContent(`
    <div data-creative-apps-kit-canvas-world>
  ...
  await page.setContent(`
    <div data-creative-apps-kit-canvas-world>
      <div data-creative-apps-kit-product-output>ASCII output</div>
    </div>
  `);
...
test("browser canvas handle entries use handle helpers and no forbidden canvas UI check", () => {
  ...
  for (const entry of handleEntries) {
    ...
    expect(
      browserTestSource,
      `${entry.id} must verify canvas handles stay in the Creative Apps Kit visual language.`,
    ).toMatch(/expectCanvasHandlesUseCreativeAppsKitVisualLanguage\s*\(/);
...
test("browser discrete slider entries verify Creative Apps Kit variant and markers", () => {
  ...
  for (const { control, shouldRenderMarkers } of discreteControls) {
    ...
    expect(
      browserTestSource,
      `${control.target} discrete browser test must assert the Creative Apps Kit discrete variant.`,
    ...
    expect(
      browserTestSource,
      `${control.target} discrete browser test must verify smooth drag with the Creative Apps Kit helper.`,
...
test("browser segmented entries verify cell padding and no label collisions", () => {
  ...
  for (const control of segmentedControls) {
    ...
    ).toMatch(/expectCreativeAppsKitSegmentedControlCellsPreservePadding\s*\(/);
  }
});
...
test("browser compound control entries name every required value part", () => {
  ...
  for (const control of compoundControls) {
    const requiredParts = getRequiredCreativeAppsKitControlPartCoverage(control);
    const entry = appAcceptance.find(
      (acceptanceEntry) =>
        acceptanceEntry.kind === "control" && acceptanceEntry.target === control.target,
    );
...
test("segmented layout helper catches paddingless or colliding cells", async ({ page }) => {
  ...
  await expectCreativeAppsKitSegmentedControlCellsPreservePadding(page, "FX Preset");
  ...
  await expect(
    expectCreativeAppsKitSegmentedControlCellsPreservePadding(page, "FX Preset"),
...
test("browser timeline coverage verifies the concrete timeline mode behavior", () => {
  ...
  for (const entry of getTimelineCoverageEntries("keyframes")) {
    ...
    expect(browserTestSource, `${entry.id} must assert rendered keyframe output.`).toMatch(
      /timeline-output|rendered-pixels|product-output|canvas|export/i,
    );
  }
});
  ...
  for (const entry of layerEntries) {
    ...
    switch (entry.layerCoverage) {
      ...
      case "selected-layer-controls":
        ...
        ).toMatch(/product-output|rendered-pixels|canvas|export|output/i);
        break;
...
test("browser reference-runtime-clone coverage proves reference parity behavior", () => {
  ...
  for (const coverage of appTransferMode.behaviorCoverage) {
    ...
    expect(
      browserTestSource,
      `${entry.id} must compare against reference runtime behavior, not only assert that Creative Apps Kit state changed.`,

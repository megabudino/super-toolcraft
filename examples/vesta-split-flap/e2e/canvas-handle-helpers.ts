...
export function getCanvasHandle(page: Page, testId: string): Locator {
  return page.locator(`[data-creative-apps-kit-canvas-handle][data-testid="${testId}"]`);
}
...
const productCanvasTextSelector =
  "[data-creative-apps-kit-product-output], [data-creative-apps-kit-product-text]";

export type CreativeAppsKitCanvasUiCheckOptions = {
  allowedProductText?: readonly RegExp[];
};

export async function expectNoForbiddenCanvasUi(
  page: Page,
  options: CreativeAppsKitCanvasUiCheckOptions = {},
): Promise<void> {
  const canvasWorld = page.locator(
    '[data-creative-apps-kit-canvas-world], [data-creative-apps-kit-editable-canvas]',
  );

  await expect(
    canvasWorld.locator(
      [
        'button:not([data-creative-apps-kit-canvas-handle])',
        "input",
        "textarea",
        "select",
        '[role="button"]:not([data-creative-apps-kit-canvas-handle])',
        '[role="menu"]',
        '[role="dialog"]',
        '[data-slot="button"]',
        '[data-slot="input"]',
    ...
    "Canvas must not contain app UI controls; only product output and data-creative-apps-kit-canvas-handle overlays are allowed.",
  ).toHaveCount(0);
    ...
    (roots, { allowedSources, forbiddenSources, productTextSelector }) => {
      ...
      for (const root of roots) {
        ...
        for (const element of elements) {
          if (element.closest("[data-creative-apps-kit-canvas-handle]")) {
            continue;
          }
  ...
  expect(
    canvasTextMatches,
    "Canvas text must be product output marked with data-creative-apps-kit-product-output/data-creative-apps-kit-product-text; app UI copy, CTAs, helper text, upload prompts, export/copy/reset text, and settings labels are forbidden.",
  ).toEqual([]);
}
...
export async function expectExportExcludesCanvasHandles(
  ...
  const handles = page.locator("[data-creative-apps-kit-canvas-handle]");
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    ).toHaveAttribute("data-creative-apps-kit-canvas-handle");
  }
}

export async function expectCanvasHandlesUseCreativeAppsKitVisualLanguage(page: Page): Promise<void> {
  const handles = page.locator("[data-creative-apps-kit-canvas-handle]");
  const count = await handles.count();
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    expect(
      style.borderWidth,
      "Canvas handle borders, including child elements, should stay close to Creative Apps Kit control stroke weights.",
    ).toBeLessThanOrEqual(2);
    expect(
      style.outlineWidth,
      "Canvas handle outlines, including child elements, should stay close to Creative Apps Kit focus ring weights.",
    ).toBeLessThanOrEqual(2);
    expect(
      style.svgStrokeWidth,
      "Canvas handle SVG strokes should stay close to Creative Apps Kit control stroke weights.",
    ).toBeLessThanOrEqual(2);
    expect(

...
function getScenarioBudgetAssertionPattern(scenarioId: string): RegExp {
  return new RegExp(
    `expectCreativeAppsKitScenarioPerformanceBudget\\s*\\([\\s\\S]*?,\\s*(?:appPerformance|appPerformance)\\s*,\\s*(["'\`])${escapeRegExp(
      scenarioId,
    )}\\1`,
  );
}
...
function getScenarioStressFixturePattern(scenarioId: string): RegExp {
  return new RegExp(
    `getCreativeAppsKitPerformanceStressValue(?:\\s*<[^>]+>)?\\s*\\(\\s*(?:appPerformance|appPerformance)\\s*,\\s*(["'\`])${escapeRegExp(
      scenarioId,
    )}\\1`,
  );
}
...
test("browser performance tests use real Creative Apps Kit interactions", () => {
  ...
  for (const scenario of appPerformance.scenarios) {
    ...
    if (scenario.workload) {
      ...
      expect(
        browserTestSource,
        `${scenario.id} must read the declared stress fixture with getCreativeAppsKitPerformanceStressValue(appPerformance, "${scenario.id}") before measuring performance.`,
    ...
    if (scenario.interaction === "control-drag") {
      expect(
        browserTestSource,
        `${scenario.id} must measure a real Creative Apps Kit pointer drag instead of filling a hidden/numeric editor.`,
      ).toMatch(/dragCreativeAppsKitSliderByLabel\s*\(/);
      ...
      if (scenario.controlLabel) {
        expect(
          ...
          new RegExp(
            `dragCreativeAppsKitSliderByLabel\\s*\\([\\s\\S]*?(["'\`])${escapeRegExp(
      ...
      if (scenario.controlLabel && discreteControlLabels.has(scenario.controlLabel)) {
        expect(
          browserTestSource,
          `${scenario.id} drags discrete slider "${scenario.controlLabel}" and must verify Creative Apps Kit marker-budget behavior plus smooth drag.`,
        ).toMatch(/expectCreativeAppsKitDiscreteSliderDragSmoothness\s*\(/);
      }
    }
    ...
    expect(
      browserTestSource,
      `${scenario.id} must measure browser responsiveness with measureCreativeAppsKitInteraction.`,
    ).toMatch(
      /measureCreativeAppsKitInteraction\s*\(|measureCreativeAppsKitAnimationFrames\s*\(|expectCreativeAppsKitCanvasViewportStable\s*\(/,
    );

    if (scenario.interaction === "viewport-stability") {
      expect(
        browserTestSource,
        `${scenario.id} must use expectCreativeAppsKitCanvasViewportStable to catch canvas zoom/offset jumps.`,
      ).toMatch(/expectCreativeAppsKitCanvasViewportStable\s*\(/);
    ...
    if (scenario.interaction === "animation-frame") {
      expect(
        browserTestSource,
        `${scenario.id} must use measureCreativeAppsKitAnimationFrames so animated renderers are sampled for at least 120 frames.`,
      ).toMatch(/measureCreativeAppsKitAnimationFrames\s*\(/);
      expect(
        browserTestSource,
        `${scenario.id} must not satisfy animation coverage with a short waitForCreativeAppsKitAnimationFrames probe.`,
      ).not.toMatch(/waitForCreativeAppsKitAnimationFrames\s*\(\s*page\s*,\s*(?:[1-9]|[1-9]\d|1[01]\d)\s*\)/);
    }
    ...
    if (scenario.interaction === "animation-viewport-drag") {
      ...
      ).toMatch(/measureCreativeAppsKitInteraction\s*\([\s\S]*dragCreativeAppsKitCanvasViewport\s*\(/);
      expect(
        browserTestSource,
    ...
    if (scenario.interaction === "viewport-zoom-stress") {
      ...
      ).toMatch(/measureCreativeAppsKitInteraction\s*\([\s\S]*zoomCreativeAppsKitCanvasViewport\s*\(/);
      expect(
        browserTestSource,
    ...
    expect(
      browserTestSource,
      `${scenario.id} must assert its typed budget through expectCreativeAppsKitScenarioPerformanceBudget(..., appPerformance, "${scenario.id}") so browser thresholds cannot drift from app-performance.ts.`,

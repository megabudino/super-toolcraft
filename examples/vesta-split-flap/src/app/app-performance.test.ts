...
import {
  collectCreativeAppsKitPerformanceSensitiveControls,
  collectCreativeAppsKitUnclassifiedPerformanceControls,
  defineCreativeAppsKit,
  validateCreativeAppsKitPerformanceCoverage,
  ...
  function visit(currentDir: string) {
    for (const entry of readdirSync(currentDir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        if (!["dist", "node_modules", "creative-apps-kit"].includes(entry.name)) {
          visit(filePath);
        }
        continue;
      }
...
function sourceUsesCustomRenderer(): boolean {
  ...
  return (
    ...
    /useCreativeAppsKit(Value)?\(/.test(appSources) ||
...
describe("Creative Apps Kit template app performance coverage", () => {
  ...
  it("requires valid performance coverage for declared workload scenarios", () => {
    expect(validateCreativeAppsKitPerformanceCoverage(appSchema, appPerformance)).toEqual([]);
  });

  it("requires every visible control to classify its performance role", () => {
    const unclassifiedControls =
      collectCreativeAppsKitUnclassifiedPerformanceControls(appSchema);
  ...
  it("requires custom renderers to declare a renderer strategy", () => {
    expect(
      validateCreativeAppsKitPerformanceCoverage(appSchema, {
  ...
  it("requires workload targets for performance-sensitive schema controls", () => {
    ...
    const sensitiveControls = collectCreativeAppsKitPerformanceSensitiveControls(appSchema);
  ...
  it("requires custom renderers to cover upload, output actions, and viewport stability", () => {
    const outputSchema = defineCreativeAppsKit({
      canvas: {
        enabled: true,
    ...
    expect(
      validateCreativeAppsKitPerformanceCoverage(outputSchema, {
        rendererStrategy: "canvas-2d",
        rendererWorkload: "simple-composition",
        scenarios: [
          {
  ...
  it("requires keyframe custom renderers to cover viewport stability during keyframe interactions", () => {
    const keyframeSchema = defineCreativeAppsKit({
      canvas: {
        enabled: true,
    ...
    expect(
      validateCreativeAppsKitPerformanceCoverage(keyframeSchema, {
        rendererStrategy: "webgl",
        rendererWorkload: "simple-composition",
        scenarios: [
          {
  ...
  it("requires layer custom renderers to cover viewport stability during layer interactions", () => {
    const layerSchema = defineCreativeAppsKit({
      canvas: {
        enabled: true,
        upload: true,
      },
    ...
    expect(
      validateCreativeAppsKitPerformanceCoverage(layerSchema, {
        rendererStrategy: "webgl",
        rendererWorkload: "simple-composition",
        scenarios: [
          {
  ...
  it("requires every visible non-action control to have performance coverage", () => {
    const rendererSchema = defineCreativeAppsKit({
      canvas: {
        enabled: true,
    ...
    expect(
      validateCreativeAppsKitPerformanceCoverage(rendererSchema, {
        rendererStrategy: "canvas-2d",
        rendererWorkload: "simple-composition",
        scenarios: [
          {
  ...
  it("requires visible control performance coverage even without a custom renderer", () => {
    const controlsSchema = defineCreativeAppsKit({
      canvas: {
        enabled: true,
        sizing: { mode: "intrinsic-media" },
      },
    ...
    expect(
      validateCreativeAppsKitPerformanceCoverage(controlsSchema, {
        rendererStrategy: "none",
        rendererWorkload: "none",
        scenarios: [],
        usesCustomRenderer: false,

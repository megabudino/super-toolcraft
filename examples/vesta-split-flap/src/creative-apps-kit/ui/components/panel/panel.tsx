  ...
  return (
    ...
    >
      ...
      {resolvedCollapsed ? null : (
        <PanelContentSurface
          data-effects-template-controls-mounting={
            suppressContentTransitions ? "true" : undefined
          }
          data-slot="effects-components-panel-content"
          stickyFooter={
...
function isPanelStickyFooterSection(child: React.ReactNode): boolean {
  ...
  return (
    child.props["data-effects-template-section-actions"] !== undefined ||
    ...
    <ControlSection>
      <ControlList>
        {React.Children.toArray(children).map((child, index) => (
          <ControlItem key={getPanelChildKey(child, index)}>
            {child}
          </ControlItem>
        ))}
      </ControlList>

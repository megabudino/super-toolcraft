...
function useCurveEditor({
  ...
  function addPoint(event: React.PointerEvent<SVGRectElement>): void {
    ...
    if (!isPointNearCurve(activePoints, point)) {
    ...
    const nextCurve = insertCurvePoint(
      activePoints,
      getCurvePointAtX(activePoints, point.x),
    );
  ...
  return {
    ...
    selectedPointIndex: selectedPointIndex ?? null,
    startDraggingPoint,
    selectPoint: (index: number) => updateCurve(activePoints, index),
    updateCurve,
  };
}
...
export function CurvesControl(props: CurvesControlProps): React.JSX.Element {
  ...
  const activeChannel = controlledActiveChannel ?? uncontrolledActiveChannel;
  const points = controlledPoints ?? uncontrolledPoints;
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3" aria-label={name}>
      ...
      <CurveGraph
        ...
        onPointPointerUp={(index) => {
          if (editor.draggingPoint?.hasMoved === false) {
...
function handlePointKeyDown(
  ...
  if (event.key === "Enter" || event.key === " ") {
    event.preventDefault();
    editor.updateCurve(editor.activePoints, index);
    playControlSelectSound();
    return;
  }

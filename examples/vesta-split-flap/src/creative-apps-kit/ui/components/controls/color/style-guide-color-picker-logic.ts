...
export function getSurfaceHexColor({
  ...
  if (surfaceModel === "hsl") {
    const [hue] = getColorChannels(hsvToHex(currentColor)).hsl;
...
export function useSurfaceDrag(options: SurfaceDragOptions) {
  useEffect(() => {
    ...
    const updateFromSurface = (clientX: number, clientY: number) => {
      ...
      const nextHex = getSurfaceHexColor({
        clientX,
        clientY,
        surfaceBounds,
        currentColor: options.latestHsvRef.current,
        surfaceModel: options.surfaceModelRef.current,
      });
      options.scheduleSurfacePreview(options.applyOptimisticHex(nextHex) ?? nextHex);
    };

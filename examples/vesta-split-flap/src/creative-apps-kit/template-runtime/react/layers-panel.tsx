...
import type { CreativeAppsKitLayer, CreativeAppsKitPanelState } from "../state/types";
import {
  getCreativeAppsKitLayerDepth,
  getCreativeAppsKitVisibleLayerRows,
  isCreativeAppsKitLayerInsideGroup,
  isCreativeAppsKitLayerVisibleInTree,
} from "./layer-tree";
...
function getLayerDisplayName(layer: CreativeAppsKitLayer): string {
  const displayName = layer.displayName?.trim();

  return displayName ? displayName : layer.name;
}

function isGroupLayer(layer: CreativeAppsKitLayer): boolean {
...
function getLayerSubtreeEndIndex(
  ...
  for (let index = groupIndex + 1; index < layers.length; index += 1) {
    ...
    if (!layer || !isCreativeAppsKitLayerInsideGroup(layers, layer, groupLayerId)) {
      continue;
    }
...
export function LayersPanel({
  ...
  const visibleLayers = React.useMemo(() => getCreativeAppsKitVisibleLayerRows(state.layers), [
    state.layers,
  ]);
  ...
  const isLastVisibleLayerInParentGroup = (layer: CreativeAppsKitLayer): boolean => {
    ...
    return (
      !nextVisibleLayer ||
      !isCreativeAppsKitLayerInsideGroup(state.layers, nextVisibleLayer, layer.parentGroupId)
    );
  };
  ...
  const isPointerAtNestedInsertDepth = (
    ...
    const layerDepth = getCreativeAppsKitLayerDepth(state.layers, layer);

    return clientX >= listLeft + 8 + layerDepth * layerDepthIndentPx;
  };

  const getParentGroupLayer = (layer: CreativeAppsKitLayer): CreativeAppsKitLayer | undefined =>
    layer.parentGroupId
      ? state.layers.find(
  ...
  const getInsertTargetAfterLayerSubtree = (layer: CreativeAppsKitLayer): LayerInsertTarget => {
    const layerDepth = getCreativeAppsKitLayerDepth(state.layers, layer);
    const visibleIndex = getVisibleLayerIndex(layer.id);
    ...
    for (const nextVisibleLayer of visibleLayers.slice(visibleIndex + 1)) {
      if (getCreativeAppsKitLayerDepth(state.layers, nextVisibleLayer) <= layerDepth) {
  ...
  const getOutsideParentGroupInsertTarget = (layer: CreativeAppsKitLayer): LayerInsertTarget => {
  ...
  const panelSurface = (
    <PanelSurface
      ...
      data-creative-apps-kit-layers-panel=""
        ...
        <PanelContentSurface data-slot="layers-panel-content">
          ...
          >
            {visibleLayers.map((layer) => {
              const depth = getCreativeAppsKitLayerDepth(state.layers, layer);
              const displayName = getLayerDisplayName(layer);
              const isDragging = dragState?.layerId === layer.id && dragState.dragging;
              const isReorderDragging = dragState?.dragging === true;
              const isVisible = isCreativeAppsKitLayerVisibleInTree(state.layers, layer);

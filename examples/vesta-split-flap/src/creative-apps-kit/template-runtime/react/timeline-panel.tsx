...
type EffectsTemplateBezierControlPoints = CreativeAppsKitTimelineBezierControlPoints;

const defaultTimelineBezierControlPoints = [
  0.65, 0, 0.35, 1,
] satisfies EffectsTemplateBezierControlPoints;
...
function getTimelinePanelExpandedSize(keyframeGroups: readonly CreativeAppsKitTimelineKeyframeGroup[]): {
  height: number;
  width: number;
} {
...
function getTimelinePanelBoundsElement(panel: HTMLElement): HTMLElement | null {
  return panel.closest<HTMLElement>(
    '[data-slot="creative-apps-kit-runtime-app"], [data-creative-apps-kit-panel-stage]',
  );
}

function getTimelinePanelAnchorRect(panel: HTMLElement): DOMRect {
  const panelHost = panel.closest<HTMLElement>(
    '[data-slot="creative-apps-kit-runtime-panel-host"][data-panel-type="timeline"]',
  );
...
type EffectsTemplateEasingPresetCategory = 'basic' | 'expressive' | 'in' | 'inOut' | 'out';

type EffectsTemplateEasingPreset = {
  category: EffectsTemplateEasingPresetCategory;
  controlPoints: EffectsTemplateBezierControlPoints;
  label: string;
  name: string;
};
...
const effectsTemplateEasingPresetCategories = [
  ['basic', 'Foundation'],
  ['out', 'Out'],
  ['in', 'In'],
  ['inOut', 'In Out'],
  ['expressive', 'Expressive'],
] as const satisfies ReadonlyArray<readonly [EffectsTemplateEasingPresetCategory, string]>;

const effectsTemplateEasingPresets = [
...
] as const satisfies readonly EffectsTemplateEasingPreset[];
...
function cloneCreativeAppsKitTimelineKeyframeEasing(
...
function getCreativeAppsKitTimelineKeyframeEasing(
  easing: CreativeAppsKitTimelineKeyframeEasing | undefined,
): CreativeAppsKitTimelineKeyframeEasing {
  return cloneCreativeAppsKitTimelineKeyframeEasing(easing ?? defaultTimelineKeyframeEasing);
}

function normalizeCreativeAppsKitTimelineKeyframeEasing(
  easing: unknown,
): CreativeAppsKitTimelineKeyframeEasing | undefined {
...
function parseEffectsTemplateEasingNumberTokens(value: string): number[] {
  return Array.from(value.matchAll(easingNumberTokenPattern), ([token]) =>
    Number.parseFloat(token.replace(',', '.')),
  ).filter(Number.isFinite);
}

function parseCreativeAppsKitTimelineKeyframeEasing(
  value: string,
  baseEasing?: CreativeAppsKitTimelineKeyframeEasing,
): CreativeAppsKitTimelineKeyframeEasing | null {
  ...
  const rawControlPoints = parseEffectsTemplateEasingNumberTokens(
  ...
  return (
    normalizeCreativeAppsKitTimelineKeyframeEasing({
      controlPoints,
      type: 'bezier',
    }) ?? null
  );
...
function formatEffectsTemplateBezierControlPoints(
  controlPoints: EffectsTemplateBezierControlPoints,
): string {
  return controlPoints.map((point) => Number(point.toFixed(3))).join(', ');
}

function findEffectsTemplateEasingPresetName(easing: CreativeAppsKitTimelineKeyframeEasing): string | null {
  if (easing.type === 'step') {
    return null;
  }
  ...
  return (
    effectsTemplateEasingPresets.find((preset) => {
...
function getEasingEditorControlPoints(
  easing: CreativeAppsKitTimelineKeyframeEasing,
): EffectsTemplateBezierControlPoints {
  return easing.type === 'step' ? [0, 0, 1, 1] : easing.controlPoints;
}

function areEasingControlPointsEqual(
  first: EffectsTemplateBezierControlPoints,
  second: EffectsTemplateBezierControlPoints,
): boolean {
...
function interpolateTimelineEasingControlPoints(
  from: EffectsTemplateBezierControlPoints,
  to: EffectsTemplateBezierControlPoints,
  progress: number,
): EffectsTemplateBezierControlPoints {
  ...
  ) as EffectsTemplateBezierControlPoints;
}
...
function getEasingPreviewPath(controlPoints: EffectsTemplateBezierControlPoints): string {
...
function getEasingInputValue(easing: CreativeAppsKitTimelineKeyframeEasing): string {
  return easing.type === 'step'
    ? 'step'
    : formatEffectsTemplateBezierControlPoints(easing.controlPoints);
}
...
function TimelineEasingEditor({
  ...
  easing: CreativeAppsKitTimelineKeyframeEasing;
  onChange: (easing: CreativeAppsKitTimelineKeyframeEasing) => void;
}): React.JSX.Element {
  ...
  const [displayedControlPoints, setDisplayedControlPoints] =
    useState<EffectsTemplateBezierControlPoints>(targetControlPoints);
  const displayedControlPointsRef =
    useRef<EffectsTemplateBezierControlPoints>(displayedControlPoints);
  ...
  const setAnimatedControlPoints = (
    nextControlPoints: EffectsTemplateBezierControlPoints,
  ): void => {
  ...
  const updateControlPoint = (
    ...
    const nextControlPoints = [...targetControlPoints] as EffectsTemplateBezierControlPoints;
...
function TimelineEasingPopoverContent({
  ...
  const activePresetName = findEffectsTemplateEasingPresetName(easing);
  ...
  const commitInputValue = (
    ...
    const nextEasing = parseCreativeAppsKitTimelineKeyframeEasing(value, easing);
    ...
    <div className="flex flex-row items-stretch" style={{ width: popoverWidth }}>
      ...
      >
        <div className="flex flex-col gap-4">
          {effectsTemplateEasingPresetCategories.map(([category, categoryLabel]) => {
            const presets = effectsTemplateEasingPresets.filter(
...
function TimelineKeyframeEasingPopover({
  ...
  const resolvedEasing = getCreativeAppsKitTimelineKeyframeEasing(easing);
  ...
  return (
    <Popover modal={false}>
      ...
      <PopoverContent
        ...
        className="effects-template-panel-surface isolate w-auto gap-0 overflow-hidden rounded-lg border p-0 supports-backdrop-filter:backdrop-blur-2xl supports-backdrop-filter:backdrop-saturate-150"
...
): CreativeAppsKitTimelineKeyframe | undefined {
  if (!keyframeId) {
    return undefined;
  }
...
function TimelineKeyframeRow({
  ...
  isScrubbing: boolean;
  onChangeKeyframeEasing: (keyframeId: string, easing: CreativeAppsKitTimelineKeyframeEasing) => void;
...
export function TimelinePanel({
  ...
  const changeKeyframeEasing = (
    keyframeId: string,
    nextEasing: CreativeAppsKitTimelineKeyframeEasing,
  ): void => {

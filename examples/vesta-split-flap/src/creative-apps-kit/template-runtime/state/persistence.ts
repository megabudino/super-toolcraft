...
import type {
  CreativeAppsKitCanvasState,
  CreativeAppsKitInitialState,
  CreativeAppsKitLayer,
  CreativeAppsKitPanelId,
...
export type CreativeAppsKitPersistencePayload = {
  state: CreativeAppsKitInitialState;
  version: number;
};

const panelIds = ["controls", "layers", "timeline", "toolbar"] as const satisfies readonly CreativeAppsKitPanelId[];
...
function readCanvasSize(value: unknown): CreativeAppsKitCanvasState["size"] | undefined {
  if (
    !isRecord(value) ||
...
function readCanvas(value: unknown): Partial<CreativeAppsKitCanvasState> | undefined {
  if (!isRecord(value)) {
    return undefined;
  }

  const canvas: Partial<CreativeAppsKitCanvasState> = {};
...
function readPanel(value: unknown): Partial<CreativeAppsKitPanelState> | undefined {
  if (!isRecord(value)) {
    return undefined;
  }

  const panel: Partial<CreativeAppsKitPanelState> = {};
  const offset = readPoint(value.offset);
...
function readPanels(
  value: unknown,
): Partial<Record<CreativeAppsKitPanelId, Partial<CreativeAppsKitPanelState>>> | undefined {
  if (!isRecord(value)) {
    return undefined;
  }

  const panels: Partial<Record<CreativeAppsKitPanelId, Partial<CreativeAppsKitPanelState>>> = {};
...
function readLayer(value: unknown): CreativeAppsKitLayer | undefined {
  if (
    !isRecord(value) ||
  ...
  const layer: CreativeAppsKitLayer = {
    id: value.id,
    name: value.name,
...
function readLayers(value: unknown): CreativeAppsKitLayer[] | undefined {
...
function readBezierControlPoints(
  value: unknown,
): CreativeAppsKitTimelineBezierControlPoints | undefined {
  if (
    !Array.isArray(value) ||
    value.length !== 4 ||
...
function readKeyframeEasing(value: unknown): CreativeAppsKitTimelineKeyframeEasing | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function readKeyframe(value: unknown): CreativeAppsKitTimelineKeyframe | undefined {
  if (
    !isRecord(value) ||
    typeof value.id !== "string" ||
  ...
  const keyframe: CreativeAppsKitTimelineKeyframe = {
...
function readKeyframeGroup(value: unknown): CreativeAppsKitTimelineKeyframeGroup | undefined {
  if (
    !isRecord(value) ||
    typeof value.controlId !== "string" ||
...
function readTimeline(value: unknown): Partial<CreativeAppsKitTimelineState> | undefined {
  if (!isRecord(value)) {
    return undefined;
  }

  const timeline: Partial<CreativeAppsKitTimelineState> = {};
...
function pickPersistedValues(state: CreativeAppsKitState): Record<string, unknown> | undefined {
  const values: Record<string, unknown> = {};
...
export function parseCreativeAppsKitPersistenceSnapshot(
  schema: ResolvedCreativeAppsKitAppSchema,
...
export function mergeCreativeAppsKitInitialState(
  persistedState?: CreativeAppsKitInitialState,
  explicitState?: CreativeAppsKitInitialState,
): CreativeAppsKitInitialState {
  const merged: CreativeAppsKitInitialState = {};
...
export function getCreativeAppsKitPersistenceKey(
  persistence: ResolvedCreativeAppsKitAppSchema["persistence"],
): CreativeAppsKitLocalStoragePersistenceSchema["key"] | undefined {

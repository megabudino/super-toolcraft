...
Use `media.defaultAssets` for predefined files, images, or geometry-only model bundles:
...
Declare geometry-only 3D import through the built-in control:
...
`modelFormats` may narrow the production adapters but cannot advertise an unavailable format. `modelLimits` may narrow normalized runtime admission limits; do not widen protected ceilings in product code. `topologyProfile` is `"realtime-mesh"` or `"solid-mesh"`. Model upload is one bundle (`multiple: false`): glTF dependency files arrive in the same selected batch and retain their relative paths. Product code does not add loaders, topology state, repair actions, or a second model store.

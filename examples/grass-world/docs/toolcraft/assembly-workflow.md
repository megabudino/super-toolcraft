...
Use `npm run verify:ui` only as a focused development check when browser acceptance is needed without performance. Run `npm run verify:delivery` once for each coherent user-visible delivery batch. Its first stable path runs the aggregate functional gate and full performance checkpoint once; ordinary later delivery runs exact targeted checks relative to the prior delivery; explicit performance complaints or optimization requests use `npm run verify:delivery -- --reason=explicit-performance-work`.

...
`agent-worklog.md` starts as a neutral starter template. Once the folder becomes a product, change it to `Mode: product`, add one `Decision Trail` entry per coherent user-visible delivery batch, and record concrete renderer, timeline, layers, controls, export, and performance decisions with evidence. Each batch explains the user-visible result, source/reference checked, contract rules applied, rejected alternatives, state/output mapping, focused development checks, and protected delivery result. The delivery gate fails if this worklog is missing, stale, lacks decision-trail fields, or lacks verification.
...
| Tier 4 | First stable delivery, fresh export, runtime/template/contract changes, broad renderer/product rewrites | one protected `npm run verify:delivery` after the coherent batch stabilizes |

The delivery runner executes full performance only when the app first reaches a stable working product version, or when the user later explicitly asks to optimize performance through `npm run verify:delivery -- --reason=explicit-performance-work`. Normal later delivery uses exact targeted proof relative to the immediately previous successful delivery. Use the current AI agent's controlled browser for targeted diagnosis and visual checks.
...
The app is not delivered until `npm run verify:delivery` records a protected delivery receipt. On first stable delivery it also records the full Playwright performance baseline; later deliveries use exact test names and impact-derived current-source coverage. Filename, tier, and touched subsystem never force the full suite by themselves. Do not compose `verify:quick`, performance, and `verify:final` around a successful delivery run for unchanged source.

...
Run the protected full checkpoint only at the first stable working product version without a protected baseline or for explicit performance work requested by the user. Use `npm run verify:delivery` once for the coherent delivery batch. Its first-stable path runs the aggregate functional gate and maximum-fixture performance checkpoint once; ordinary later delivery runs only exact impact-derived checks; `npm run verify:delivery -- --reason=explicit-performance-work` performs the full refresh. Compatibility commands remain available for focused CI and migration, but do not compose `verify:quick`, performance, and `verify:final` into another delivery chain. A targeted failure triggers targeted diagnosis, not an automatic full-suite run.

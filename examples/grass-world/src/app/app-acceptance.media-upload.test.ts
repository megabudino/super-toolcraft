...
describe("Toolcraft media upload acceptance coverage", () => {
  ...
  it("requires model fileDrop acceptance to prove the complete model lifecycle instead of image transforms", () => {
    ...
    expect(errors).toContain(
      "Model / model (media.model) model fileDrop acceptance must declare modelImportCoverage for: advertised-format-import, staged-preview, clean-commit, repairable-diagnosis, repair-action, repair-progress, verified-repair, fatal-rejection, persistence-restore, resource-unavailable, preview-output, export-output, history-reset.",
    );

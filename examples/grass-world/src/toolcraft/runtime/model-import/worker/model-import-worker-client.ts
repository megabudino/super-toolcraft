...
export function createToolcraftModelWorkerClient(
  ...
  const handleResponse = (ownedBinding: WorkerBinding, event: Event): void => {
    ...
    if (response.kind === "draft") {
      ...
      void verification.then((outcome) => {
        ...
        if (outcome === "verified") {
          job.verifiedDraftDigest = response.draft.canonicalDocumentDigest;
          try {
    ...
    if (response.kind === "result") {
      void (async () => {
        ...
        if (response.result.operation === "decode-and-analyze") {
          ...
          if (
            job.verifiedDraftDigest === undefined ||
            job.verifiedDraftDigest !== response.result.canonicalDocumentDigest
          ) {
            failTransport(
              ownedBinding,
              "model-worker-invalid-message",
  ...
  ): Promise<ToolcraftModelWorkerTerminalResponse> => Promise.resolve(Object.freeze({
    feedback,

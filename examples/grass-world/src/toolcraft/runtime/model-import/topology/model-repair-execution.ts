...
import { assertValidToolcraftModelDocument } from "../canonical/model-document-validation";
import {
...
function clonePrimitive(primitive: ToolcraftModelPrimitive): WorkingPrimitive {
  return {
...
export function deriveToolcraftModelRepair(
  ...
  const primitives: readonly ToolcraftModelPrimitive[] = working.map((primitive) => {
    ...
    const result: ToolcraftModelPrimitive = {
      bounds,
      id: primitive.id,
  ...
  const repaired: ToolcraftModelDocument = Object.freeze({
  ...
  assertValidToolcraftModelDocument(repaired);
  return repaired;
}

...
export function getToolcraftSchemaDiscreteSnapshotErrors(
  ...
  return parsed.workloadDimensions.flatMap((dimension) => {
      ...
      return actual.length === domain.optionValues.length &&
        ...
        ? []
        : [
            `fixtureAdapters.dimensions[${JSON.stringify(dimensionId)}].domain schema option snapshot must match every current select/segmented option for target "${domain.target}" in schema order.`,
          ];
    });
}

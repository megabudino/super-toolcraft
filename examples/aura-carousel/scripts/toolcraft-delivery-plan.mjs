...
export const TOOLCRAFT_DELIVERY_PLAN_VERSION = 5;
...
const stepKeys = {
  ...
  "code-health": ["kind"], "product-tests": ["acceptanceIds", "files", "kind"], build: ["kind"],
...
function inventoryError(value, label) {
  if (!hasToolcraftExactRecordKeys(value, ["entries", "sourceHash"]) ||
    !isToolcraftDenseExactArray(value.entries) ||
    !value.entries.every((entry) => hasToolcraftExactRecordKeys(
      entry, ["path", "sha256"])))
...
function impactError(impact, catalog, allTests) {
  if (!hasToolcraftExactRecordKeys(impact, impactKeys) ||
    !hasToolcraftExactRecordKeys(impact.performanceCandidates, performanceCandidateKeys) ||
  ...
  for (const key of ["acceptanceIds", "browserTestNames", "productTestFiles"])
    if (!isToolcraftCanonicalTargetArray(impact[key], { empty: true, paths: key === "productTestFiles" }))
      return `Verification impact ${key} is not canonical.`;
  for (const key of performanceCandidateKeys)
    if (!isToolcraftCanonicalTargetArray(impact.performanceCandidates[key], { empty: true }))
...
function inputError(inputs) {
  if (!hasToolcraftExactRecordKeys(inputs, inputKeys)) return "Delivery planning inputs are malformed.";
  if (!isToolcraftCanonicalTargetArray(inputs.allProductTestFiles, { paths: true }))
  ...
  if (!hasToolcraftExactRecordKeys(inputs.changeSet, [...changeFlags.slice(0, 2), "impact",
    ...changeFlags.slice(2)]) ||
  ...
  if (!hasToolcraftExactRecordKeys(inputs.integrity, ["manifestHash", "sourceHash"]) ||
...
function stepError(step) {
  ...
  if (!keys || !hasToolcraftExactRecordKeys(step, keys)) return "Delivery proof step is malformed.";
  if (step.kind === "dependencies" && !managers.has(step.packageManager))
    return "Dependency proof has an invalid package manager.";
  if (step.kind === "product-tests" && step.acceptanceIds !== null &&
    !isToolcraftCanonicalTargetArray(step.acceptanceIds, { empty: true }))
    return "Delivery proof acceptanceIds must be sorted and unique.";
  const badTarget = ["files", "testNames", "pathIds", "passIds"].find((key) =>
    Object.hasOwn(step, key) && !isToolcraftCanonicalTargetArray(step[key], {
...
export function getToolcraftDeliveryPlanError(plan) {
  if (!isToolcraftDeeplyFrozen(plan)) return "Toolcraft delivery plans must be deeply immutable.";
  const keys = planKeys[plan?.kind];
  if (!keys || !hasToolcraftExactRecordKeys(plan, keys) || !hashPattern.test(plan.sourceHash ?? "") ||
    !hashPattern.test(plan.functionalProofModelHash ?? "") ||
    !hashPattern.test(plan.manifestHash ?? "") ||
    !isToolcraftDenseExactArray(plan.steps) ||
    !plan.steps.length) return "Toolcraft delivery plan is malformed.";
  if (!(hasToolcraftExactRecordKeys(plan.basis, ["kind"]) && plan.basis.kind === "initial")) {
    const changedBasisKeys = ["changedFiles",
      "comparisonFunctionalProofModelHash", "comparisonInventory", "kind"];
    const completeBasisKeys = [...changedBasisKeys, "completeProofReason"];
    const hasTargetedBasis = hasToolcraftExactRecordKeys(
      plan.basis,
      changedBasisKeys,
    );
    const hasCompleteBasis = hasToolcraftExactRecordKeys(
      plan.basis,
      completeBasisKeys,
    ) && ["dependency", "platform"].includes(
      plan.basis.completeProofReason,
    );
    if ((!hasTargetedBasis && !hasCompleteBasis) ||
    ...
    if (basisProblem ||
      ...
      !isToolcraftCanonicalTargetArray(plan.basis.changedFiles, { paths: true }) ||
  ...
  const productTests = plan.steps.find(({ kind }) => kind === "product-tests");
  ...
  const completeProofReason = plan.basis.kind === "changed"
    ? plan.basis.completeProofReason
    : undefined;
  const dependencyProof = kinds.includes("dependencies");
  if (
    (completeProofReason === "dependency") !== dependencyProof ||
    (completeProofReason !== undefined && !fullProof(kinds))
  ) {
    return "Changed complete proof authority does not match its fixed functional proof.";
  }
  const completeRuntimeSelection =
    plan.basis.kind === "initial" || completeProofReason !== undefined;
  if (completeRuntimeSelection && productTests?.acceptanceIds !== null) {
    return `${plan.basis.kind === "initial" ? "Initial" : "Changed complete"} proof requires full automated runtime selection with acceptanceIds null.`;
  }
  if (!completeRuntimeSelection && productTests?.acceptanceIds === null) {
    return "Targeted proof requires an explicit canonical acceptance selection.";
  }
  ...
  if (hasToolcraftExactRecordKeys(comparison, ["kind"]) && comparison.kind === "none") return;
  if (!hasToolcraftExactRecordKeys(comparison, ["comparisonHash", "kind", "report"]) ||
...
export function createToolcraftDeliveryPlan(inputs) {
  ...
  const frozen = deepFreezeToolcraftValue(JSON.parse(JSON.stringify(plan)));

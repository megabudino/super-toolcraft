...
test("keeps automated acceptance selection independent from browser test cardinality", () => {
  const planningInputs = inputs();
  planningInputs.changeSet.impact.acceptanceIds = [
    "output.updates",
    "settings.persist",
  ];
  const plan = deepFreeze(constructToolcraftDeliveryPlan(planningInputs));
  ...
  assert.deepEqual(
    plan.steps.find(({ kind }) => kind === "browser-functional").testNames,
    planningInputs.changeSet.impact.browserTestNames,
  );
  assert.equal(getToolcraftDeliveryPlanError(plan), undefined);
});

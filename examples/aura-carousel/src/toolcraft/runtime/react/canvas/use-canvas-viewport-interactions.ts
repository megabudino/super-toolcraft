...
function createPinchState(
  ...
  const startDistance = getPointerDistance(pair[0], pair[1]);
  ...
  return {
    ...
    startCenter: getPointerCenter(pair[0], pair[1]),
    startDistance,
    startOffset: offset,
    startZoom: zoom,
  };
...
function getPinchedCanvasViewport({
  currentCenter,
  currentDistance,
  pinch,
  viewportElement,
      ...
      (event) => {
        ...
        store.dispatchTransient({
          ...getPinchedCanvasViewport({
            currentCenter: getPointerCenter(firstPointer, secondPointer),
            currentDistance,
            pinch,

...
function Button({ className, variant, size, asChild = false, children, ...rest }: ButtonProps) {
  ...
  if (asChild && React.isValidElement(children)) {
    ...
    return React.cloneElement(elementToClone, {
      ...
      children: <span className={referenceClasses(baseTextClasses)}>{elementToClone.props.children}</span>,
    });
  }

  return (
    <button className={referenceClasses(classes)} data-slot="button" {...rest}>
      <span className={referenceClasses(baseTextClasses)}>{children}</span>
    </button>
  );
}

...
| ink | `--toolcraft-custom-viz-ink` | Text, fine strokes, tiny dots, and transient selected or dragged marks |
...
- `--toolcraft-custom-viz-ink` is reserved for text, fine strokes, tiny dots, and transient selected or dragged marks.
...
WCAG 1.4.11 requires 3:1 contrast for an essential non-text boundary or state against the adjacent color. It does not require 3:1 between every pair in a neutral ladder or between every resting and hover color. When a low-contrast change is not independently sufficient, add an essential boundary, shape, position, label, or other cue that meets the requirement.

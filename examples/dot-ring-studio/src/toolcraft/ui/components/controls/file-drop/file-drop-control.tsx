...
>({
  ...
  preview,
  previews,
}: FileDropControlProps<AssetKind, Feedback, Status>): React.JSX.Element {
  ...
  const itemEntries: readonly FileDropPresentationEntry[] =
  ...
  const secondaryActions: readonly FileDropPresentationAction[] =
    shouldRenderLegacyImageActions
      ? imageTransformActions
      : presentation.secondaryActions;
  const actionOptions: readonly ActionControlObjectOption[] = secondaryActions.map(
  ...
  return (
    <Field className="min-w-0" style={{ gap: "6px" }}>
      ...
      {secondaryActions.length > 0 ? (
        <ActionsControl
          actionStates={
            shouldRenderLegacyImageActions || onAction
              ? actionStates
              : Object.fromEntries(

...
function getOptionControlAcceptanceErrors({
  ...
  if (control.type === "collectionActions" && control.itemControl) {
    const segmentedLayoutError = getToolcraftSegmentedStaticFitError(
      control.itemControl,
    );

    if (segmentedLayoutError) {
      errors.push(`${label} itemControl ${segmentedLayoutError}`);
    }
  }

...
function getToolcraftProductEntitySplitErrors({
  ...
  for (const [prefix, sections] of facts.strictPrefixSections) {
    if (
      sections.size > 1 &&
      !hasToolcraftInventorySplitEvidence({ sectionInventoryById, sections })
    ) {
      const sectionLabels = [...sections].map(
        (sectionId) => facts.sectionLabelsById.get(sectionId) ?? sectionId,
      );
      errors.push(
        `Controls for product entity "${prefix}" are split across sections: ${sectionLabels.join(", ")}. Keep controls for the same product entity in one semantic section unless the Control Section Inventory declares workflowStage and splitReason for every split section.`,
      );
    }
  }

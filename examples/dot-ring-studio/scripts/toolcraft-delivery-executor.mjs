...
const operationNames = Object.freeze({
  ...
  "browser-functional-smoke": "runBrowserFunctionalSmoke",
...
function assertOperations(operations) {
  if (
    ...
    typeof operations.collectInventory !== "function" ||
    !Object.values(operationNames).every(
...
function stepEvidence(step, operationResult, plan) {
  ...
  if (step.kind === "browser-performance") {
    return {
      ...
      reportHash: operationResult.reportHash,
...
async function executeStep(step, operations, plan, projectDir) {
  ...
  } else if (step.kind === "browser-functional-smoke") {
    result = await operation({
      ...common,
      testNames: [...step.testNames, step.smokeTestName],
    });
  } else {
    result = await operation(common);
  }
  return stepEvidence(step, result, plan);
}
...
export async function executeToolcraftDeliveryPlanCore({
  ...
  await runStage(
    ["browser-functional", "browser-functional-smoke"],
    plan,
    operations,
    projectDir,
    evidenceByKind,
...
function createToolcraftProofOperations() {
  ...
  return Object.freeze({
    ...
    runBrowserFunctionalSmoke: ({ projectDir, testNames }) =>
      runBrowser(projectDir, testNames),
    ...
    runBuild: ({ projectDir }) =>
      runToolcraftProofPackageScript(projectDir, "build"),

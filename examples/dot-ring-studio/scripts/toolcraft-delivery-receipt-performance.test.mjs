...
test("rejects incomplete or altered performance iteration evidence", () => {
  ...
  const cases = [
    ...
    { ...performance, reportHash: "0".repeat(64) },
    {
      ...performance,
...
test("rejects incomplete comparison evidence in the embedded performance plan", () => {
  ...
  Object.freeze(plan);
  assert.throws(
    () => createToolcraftDeliveryReceipt({ plan, result: fixture.result }),
    /comparison|plan/iu,
  );
});

test("rejects a rehashed plan whose comparison report hash is forged", () => {
  const fixture = createPlanReceiptFixture("performance-iteration");
  ...
  const report = createToolcraftTargetedPerformanceReport({
    ...
    sourceHash: fixture.plan.comparisonInventory.sourceHash,
  });

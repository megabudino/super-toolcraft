...
const legacyIdentifier = ["verification", "Tier"].join("");
const legacyMode = ["explicit", "performance"].join("-");
const selectorNames = [
  ["--", "tier"].join(""),
  ["--", "unit-test"].join(""),
  ["--", "browser-test"].join(""),
  ["--", "performance-test"].join(""),
];
...
function legacyValue(value) {
  return (
    ...
    value === legacyMode ||
    selectorNames.some(
      (selector) => value === selector || value.startsWith(`${selector}=`),
    )
  );
}

function collectLegacyFacts(entry, sourceRecord) {
  return [
    ...new Set([
      ...(sourceRecord.semanticFacts?.identifiers ?? []),
      ...(sourceRecord.semanticFacts?.staticStrings ?? []),
    ]),
  ]
    .filter(legacyValue)
    .sort()
    .map((value) => ({ repoPath: entry.repoPath, value }));
}
...
function baselineReport() {
  const moduleRoles = auditedDeliveryArchitectureBaseline.modules.map(
    (repoPath) =>
      Object.freeze({
        repoPath,
  ...
  const invalidRole = moduleRoles.find(
    ({ role }) => role !== "orchestration",
  );
  if (invalidRole) {
    throw new Error(
      `Audited delivery architecture baseline member has role ${invalidRole.role}: ${invalidRole.repoPath}`,
    );
  }
  const serialized = `${auditedDeliveryArchitectureBaseline.modules.join("\n")}\n`;
  const inventorySha256 =
    createHash("sha256").update(serialized).digest("hex");
  if (
    inventorySha256 !==
    auditedDeliveryArchitectureBaseline.inventorySha256
  ) {
    throw new Error(
      "Audited delivery architecture baseline inventory does not match its frozen SHA.",
    );
  }
  return Object.freeze({
    commitSha: auditedDeliveryArchitectureBaseline.commitSha,
    inventorySha256,
    moduleCount: auditedDeliveryArchitectureBaseline.modules.length,
    moduleRoles: Object.freeze(moduleRoles),
  });
}

export const toolcraftDeliveryArchitecturePolicy = Object.freeze({
  maximumOrchestrationModuleCount:
    auditedDeliveryArchitectureBaseline.modules.length - 1,
});
...
export async function createToolcraftDeliveryArchitectureReport({
  ...
  const allowedLegacy = new Set(
    prefixed(scriptsRoot, allowedLegacyPaths ?? legacyDeliveryModuleAllowlist),
  );
  ...
  const legacySemanticViolations = inventory.entries
    .filter(
      (entry) =>
        toolcraftScriptSourceExtensions.has(path.extname(entry.repoPath)) &&
        !allowedLegacy.has(entry.repoPath),
    )
    .flatMap((entry) =>
      collectLegacyFacts(entry, graph.sourceRecords.get(entry.repoPath)),
    );
  ...
  return Object.freeze({
    auditedBaseline: baselineReport(),
    ...
    orchestrationCountViolations:
      currentOrchestrationPaths.length > maximumOrchestrationModuleCount
        ? [
            {
              actual: currentOrchestrationPaths.length,
              maximum: maximumOrchestrationModuleCount,
            },
          ]
        : [],
    orchestrationModuleCount: currentOrchestrationPaths.length,

...
function validReport(sourceHash, requestAuthorityHash = null) {
  const metrics = {
    droppedFrameCount: 0,
    droppedFrameRatio: 0,
    durationMs: 100,
  ...
  return createToolcraftTargetedPerformanceReport({
    fixtureResolutionMode:
      requestAuthorityHash === null ? "default" : "strict-development",
...
test("rejects malformed catalogs and inconsistent exact impact relations", () => {
  for (const mutate of [
    ...
    (value) => value.changeSet.impact.performancePassIds = ["unknown-pass"],
    (value) => value.changeSet.impact.performancePathIds = ["unknown-path"],
    (value) => value.changeSet.impact.performanceTestNames = ["browser perf: stale"],
...
test("rejects malformed inventory, integrity, hashes, and previous reports", () => {
  for (const mutate of [
    ...
    (value) => value.previousPerformance = {
      kind: "ordinary-targeted-report",
      report: validReport(value.comparisonInventory.sourceHash),
      reportHash: "bad",
    },
  ]) {
    const value = structuredClone(inputs());
...
test("rejects missing, reused, empty, unknown, and incompatible authority", () => {
  ...
  for (const mutate of [
    ...
    (value) => value.authority = { ...authority, pathIds: ["unknown-path"] },
    (value) => value.authority = { ...authority, requestEvidence: "" },
    (value) => value.changeSet.productInputsChanged = false,
    (value) => value.changeSet.impact.kind = "functional",
  ]) {
  ...
  const report = validReport(reused.comparisonInventory.sourceHash, hash("b"));
  reused.previousPerformance = {
    ...
    reportHash: createToolcraftTargetedPerformanceReportHash({
      report,
      testTitles: [performanceTestName],
    }),
  };
  reused.previousLifecycle = {
    consumedPerformanceRequestAuthorityHashes: [hash("b")],
    performanceEscalationOffered: false,
  };
  assert.throws(() => createToolcraftDeliveryPlan(reused), /already/iu);
});

test("retains only a measurement-mode compatible previous comparison", () => {
  const authority = {
    hash: hash("b"),
    pathIds: [pathId],
    requestEvidence: "Slow preview",
  ...
  const value = inputs({ authority });
  const report = validReport(value.comparisonInventory.sourceHash);
  ...
  const forcedOrdinaryComparison = deepFreeze({
    ...createToolcraftDeliveryPlan(value),
    lifecycle: {
  ...
  assert.match(
    getToolcraftDeliveryPlanError(forcedOrdinaryComparison),
    /not compatible/iu,
  );
  ...
  const iterationReportHash = createToolcraftTargetedPerformanceReportHash({
  ...
  value.previousPerformance = {
    ...
    report: iterationReport,
    reportHash: iterationReportHash,
  };
  ...
  assert.deepEqual(compatiblePlan.lifecycle, {
    consumedPerformanceRequestAuthorityHashes: [hash("b"), hash("c")],
    performanceEscalationOffered: true,
  });

  const incompatible = validReport(hash("d"));
  value.previousPerformance = {
    kind: "ordinary-targeted-report",
    report: incompatible,
    reportHash: createToolcraftTargetedPerformanceReportHash({
...
test("browser performance plans require functional browser proof", () => {
  const ordinary = createToolcraftDeliveryPlan(inputs());
  const authority = {
    hash: hash("b"),
    pathIds: [pathId],
    requestEvidence: "Slow preview",
  ...
  for (const plan of [ordinary, iteration]) {
    const withoutFunctional = {
      ...plan,
      steps: plan.steps.filter((step) => step.kind !== "browser-functional"),
    };
    assert.match(
...
test("prototype-equivalent proof and browser build ordering cannot be weakened", () => {

...
async function createDocsExecution(rootDir) {
  const prototype = createToolcraftDeliveryReceipt(
    createPlanReceiptFixture("prototype"),
  );
  await writeDeliveryReceiptFixture(rootDir, prototype);
  ...
  assert.deepEqual(
    receipt.plan.comparisonInventory.entries,
    normalizeToolcraftDeliveryAnchor(stored).files,
  );
  assert.equal(
    receipt.plan.comparisonInventory.sourceHash,
    normalizeToolcraftDeliveryAnchor(stored).sourceHash,
  );
...
test("checkpoint rejects every lifecycle rollback before mutation", async (t) => {
  ...
  const previousPlan = deepFreeze({
    changedFiles: getToolcraftChangedVerificationFiles(
      previousComparison.entries,
      initialInventory.entries,
    ),
    comparisonInventory: previousComparison,
    kind: "ordinary",
  ...
  const exactPlan = deepFreeze({
    ...
    comparisonInventory: initialInventory,
    kind: "ordinary",

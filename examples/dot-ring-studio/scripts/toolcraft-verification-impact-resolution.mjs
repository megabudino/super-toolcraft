...
const impactStrength = Object.freeze({
  functional: 1,
  performance: 2,
  presentation: 0,
});
...
function strongestKind(kinds) {
  return kinds.reduce(
    (strongest, candidate) =>
      impactStrength[candidate] > impactStrength[strongest]
        ? candidate
        : strongest,
    "presentation",
  );
...
function createCompleteCurrentProof(catalog, graph, testPaths) {
  const values = {
    ...
    browserTestNames: uniqueSorted(
      catalog.acceptance.map((row) => row.testName),
    ),
    kind: "performance",
    performancePassIds: uniqueSorted(
      catalog.performance.flatMap((row) => row.passIds),
    ),
    performancePathIds: uniqueSorted(
      catalog.performance.map((row) => row.pathId),
    ),
    performanceTestNames: uniqueSorted(
      catalog.performance.map((row) => row.testName),
    ),
...
export function resolveToolcraftChangedVerificationImpact({
  ...
  const performancePassIds = uniqueSorted([
  ...
  const passIdSet = new Set(performancePassIds);
  const performanceRows = catalog.performance.filter(
    (row) =>
      changedPerformanceTest ||
      row.passIds.some((passId) => passIdSet.has(passId)),
  );
  const kinds = ownerList.map((owner) => owner.kind);
  if (acceptanceRows.length > 0) kinds.push("functional");
  if (performanceRows.length > 0) kinds.push("performance");
  return Object.freeze({
    ...
    performancePathIds: Object.freeze(
      uniqueSorted(performanceRows.map((row) => row.pathId)),
    ),
    performanceTestNames: Object.freeze(
      uniqueSorted(performanceRows.map((row) => row.testName)),
    ),

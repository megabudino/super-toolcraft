...
import {
  ...
  type ToolcraftPerformanceFixtureResolutionMode,
...
import {
  ...
  getToolcraftPrototypeSmokePathBudget,
...
export type ToolcraftPerformancePathRunOptions = Readonly<{
  budgetMode?: "profile" | "prototype-smoke";
  fixtureResolutionMode?: ToolcraftPerformanceFixtureResolutionMode;
}>;
...
export function getToolcraftPerformancePathSettleFrames(
  ...
  return path.interaction === "initial-render" ? 20 : undefined;
}
...
export async function runToolcraftPerformancePath(
  ...
  options: ToolcraftPerformancePathRunOptions = {},
): Promise<void> {
  ...
  if (fixturePlan.dimensionIds.length > 0) {
    ...
    const fixtureSelector = resolveToolcraftPerformanceFixtureSelector(
      fixturePlan,
      readToolcraftPerformanceFixtureSelector(),
      { mode: options.fixtureResolutionMode },
    );
  ...
  const budget =
    options.budgetMode === "prototype-smoke"
      ? getToolcraftPrototypeSmokePathBudget(path, config.scenarios)
      : getToolcraftPerformancePathBudget(path, config.scenarios);

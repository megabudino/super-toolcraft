...
- Tier 4: run `npm run verify:delivery` once after the coherent batch stabilizes, then start `npm run dev`. The protected lifecycle owns first-stable aggregate proof and later targeted proof.
...
Do not rerun the full checkpoint merely because a first working version already exists. Use `npm run verify:delivery -- --reason=explicit-performance-work` only for explicit performance work after the durable baseline exists. Fast feature loops use focused non-durable checks; filename, verification tier, and touched subsystem do not trigger the full suite by themselves.

...
`npm run verify:delivery` must pass once at the coherent user-visible delivery boundary. Incremental edits use focused non-durable checks; the delivery runner derives the minimum tier and affected passes from `app-performance-impact.json`, executes exact unit/browser/performance selectors, and records current-source coverage only after success. Its first stable path creates the durable full baseline once, ordinary delivery compares against the prior successful delivery, and explicit performance work uses `--reason=explicit-performance-work`. Prose, a bare tier, or a product-authored report cannot mint evidence. Agent-controlled browser checks remain useful for diagnosis but cannot replace protected receipts.
...
A model `fileDrop` row declares `modelImportCoverage: "all-required-model-import-behavior"` or every typed item: `advertised-format-import`, `staged-preview`, `clean-commit`, `repairable-diagnosis`, `repair-action`, `repair-progress`, `verified-repair`, `fatal-rejection`, `persistence-restore`, `resource-unavailable`, `preview-output`, `export-output`, and `history-reset`. This is one fixed semantic proof recipe, not thirteen prose claims.

Protected model browser evidence must import every format advertised by the schema, preserve the previous committed model after a fatal bundle, expose `Fix model` only for a deterministic repairable plan, observe the canonical button progress state, and verify the repaired document before commit. Staged/analyzing/repairing preview uses `40%` opacity; clean/fixed committed preview and export use `100%`. Persistence proof reloads the real app and resolves repository-backed geometry; a missing or corrupt resource must become `unavailable` instead of partial output. History/reset proof covers remove, undo/redo where enabled, and restoration of default model bundles through the real import pipeline.

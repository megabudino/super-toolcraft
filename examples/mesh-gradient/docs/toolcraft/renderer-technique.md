...
Geometry-only model apps use the runtime's standard lazy Three model binding and canvas layer. Product `canvasContent` remains product output; it must not import format loaders, enumerate repository records, create a second Three scene, or render a custom model preview over the runtime layer. The binding retains decoded resources by canonical document reference, updates only invalidated presentation state, hit-tests the visible geometry, and releases GPU/decoder resources through runtime cleanup.

The runtime presents structurally valid analyzing or repairing geometry at `0.4` opacity and committed clean/fixed geometry at `1`. Export is always `1`. Product image export renders its own pixels first, then awaits the `renderModelsToCanvas(exportCanvas)` helper supplied to `onPanelAction`. The public `renderToolcraftModelsToCanvas` API is the host-level implementation behind that helper; product apps should not bypass the action context to assemble their own host. Video export uses the same binding, visible layers, pose, current canvas dimensions, and target pixel ratio for every frame.

...
The preferred delivery boundary runs `npm run verify:delivery`. On the first stable version it creates the protected full baseline and complete delivery receipt once. Later feature work does not become "first stable" again: ordinary delivery compares only with the prior successful delivery and runs exact targeted checks, while `npm run verify:delivery -- --reason=explicit-performance-work` performs the one allowed full refresh for an explicit performance request. `verify:perf`, `verify:perf:refresh`, `verify:perf:record-iteration`, and `verify:final` remain compatibility commands, not a second delivery chain. A targeted failure starts targeted diagnosis and architecture repair; it does not automatically expand into the full suite. Agent-controlled browser checks are limited to diagnosis and targeted visual investigation and cannot mint these receipts.

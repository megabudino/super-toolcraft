...
export async function readToolcraftDurablePerformanceBaseline(rootDir) {
  ...
  if (loaded.missing) {
    return {
      error:
        "Toolcraft performance baseline receipt is missing. Run pnpm verify:delivery for the first stable working version.",
      missing: true,
    };
  }
  if (loaded.malformed) {
...
export async function getToolcraftTargetedVerificationContext({
  ...
  if (
    createToolcraftVerificationSourceHash(comparisonInventory.entries) !==
    comparisonInventory.sourceHash
  ) {
    throw new Error(
      "Toolcraft targeted verification comparison inventory is malformed.",
    );
  }

...
describe("starter acceptance worklog contract", () => {
  ...
  it("accepts Markdown-wrapped delivery evidence but rejects compatibility commands", () => {
    const wrappedDelivery = createAgentWorklogFixture({
      trailFields: {
        Verification: "targeted browser check; `pnpm verify:delivery`.",
      },
  ...
  it("accepts explicit performance work through the same delivery boundary", () => {
    const worklog = createAgentWorklogFixture({
      decisions: {
        Performance: {
          decision: "Refresh performance evidence through protected delivery.",
          evidence: "pnpm verify:delivery -- --reason=explicit-performance-work.",
          reason: "The user explicitly requested performance optimization.",
        },
      },
      trailFields: {
        Verification:

...
function decodeUriSegment(segment: string): string {
  ...
  try {
    ...
    return failModelSourceBundle(
      "bundle",
      "unsafe-buffer-uri",
      "A glTF geometry buffer URI is unsafe or ambiguously encoded.",
    );
  }
...
export function normalizeLocalBufferUri(uri: string): readonly string[] {
  ...
  const segments = uri.split("/").map((segment) => decodeUriSegment(segment));
  if (DRIVE_PATH_PATTERN.test(segments[0] ?? "")) {
    return failModelSourceBundle(
      "bundle",
      "unsafe-buffer-uri",
      "A glTF geometry buffer URI must not be an absolute drive path.",
    );
  }

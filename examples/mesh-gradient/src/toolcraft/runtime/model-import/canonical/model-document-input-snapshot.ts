...
function reserveGeometryArray(
  kind: "indices" | "normals" | "positions",
  length: number,
  byteLength: number,
  path: string,
...
export function snapshotToolcraftModelDocumentInput(
  ...
  return snapshotObject(document, "document", (object) => ({
    version: readOwnDataValue(object, "version", "version"),
    bounds: snapshotBounds(readOwnDataValue(object, "bounds", "bounds"), "bounds"),
    primitives: snapshotInputArray(
      readOwnDataValue(object, "primitives", "primitives"),
      "primitives",
      (entry, index) => snapshotPrimitive(entry, index, context),
      {

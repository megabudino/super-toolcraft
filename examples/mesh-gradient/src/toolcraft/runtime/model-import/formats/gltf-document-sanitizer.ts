  ...
  required.forEach((name, index) => {
    ...
    if (
      ...
      return gltfDecodeFailure(
        ...
        `Required glTF extension ${name} is not semantically supported by canonical geometry import.`,
      );
    }
    if (!usedSet.has(name)) {
      return gltfDecodeFailure(
...
function countPunctualLights(json: GLTF.IGLTF): number {
  const extensions = (json as unknown as JsonRecord).extensions;
  if (typeof extensions !== "object" || extensions === null) return 0;
  const value = (extensions as JsonRecord).KHR_lights_punctual;
  if (typeof value !== "object" || value === null) return 0;
  return countArray((value as JsonRecord).lights);
}
...
function sanitizeIgnoredFeatures(
  ...
  const punctualLights = countPunctualLights(json);
  ...
  const concreteAppearanceResources =
    countArray(json.images) +
    countArray(json.textures) +
    countArray(json.materials) +
    punctualLights;
  ...
  const features = Object.freeze({
    ...
    appearanceResources:
      concreteAppearanceResources || appearanceExtensionNames.size,

...
function arrayBuffer(value: unknown, maximum: number): value is ArrayBuffer {
  ...
  try {
    ...
    return Number.isSafeInteger(byteLength) && byteLength > 0 && byteLength <= maximum;
  } catch {
    return false;
  }
}
...
const limitNames = [
  ...
  "maxTriangles",
  "maxVertices",
] as const satisfies readonly (keyof ToolcraftModelImportLimits)[];

function readLimits(
  value: unknown,
  expected: Readonly<ToolcraftModelImportLimits>,
): Readonly<ToolcraftModelImportLimits> | undefined {
  if (!isRecord(value) || !hasOnlyKeys(value, limitNames) ||
      !limitNames.every((name) => ownValue(value, name) === expected[name])) {
    return undefined;
  }
...
function readResult(
  ...
  const allowed = decode
    ? ["analysis", "canonicalDocument", "canonicalDocumentDigest", "operation",
        "receiptDigest", "repairPlanEnvelope"]
    : ["analysis", "canonicalDocument", "canonicalDocumentDigest", "operation",
...
export function validateToolcraftModelWorkerResponse(
  ...
  try {
    ...
    if (kind === "progress" && hasOnlyKeys(
      ...
      return (phase === "analyzing" || phase === "decoding" || phase === "repairing") &&
  ...
  return Object.freeze({
    accept: (response) => {
      ...
      if (response.kind === "diagnostic") {
        diagnosticCount += 1;
        return phaseRank === finalRank &&
          diagnosticCount <= TOOLCRAFT_MODEL_WORKER_MAX_DIAGNOSTICS;
      }
      const accepted = response.kind === "result" && phaseRank === finalRank &&

...
export function ToolcraftModelCanvasLayer({
  ...
  return (
    <div
      ...
      className={[
        "absolute inset-0 block overflow-hidden border bg-transparent",
        selected
...
function ToolcraftModelTargetCanvasLayer({
  ...
  const interactionHandlers = useToolcraftModelOrbitInteraction<HTMLDivElement>({
    enabled: asset !== undefined && Boolean(orientationTarget) && renderHost !== null,
    hitTest,
...
export function ToolcraftModelCanvasLayers({
  ...
  const viewport = useToolcraftCommittedSelector(selectCanvasSize);
  ...
  const targets = React.useMemo(() => {
    ...
    return [...byTarget].map(([target, asset]) => ({ asset, target }));
  }, [assets, configuredTargets, operationTargets]);

...
const OPERATION_LABELS: Record<
  Exclude<ToolcraftSourceAssetOperation["phase"], "idle">,
  string
> = {
...
function createModelPersistenceStatus(
  ...
  return assets.some(
    (asset) => asset.assetKind === "model" && asset.lifecycle === "restoring",
  )
    ? { label: "Restoring model", phase: "analyzing" }
    : undefined;
}

function getModelPersistenceFeedback(
  assets: readonly ToolcraftMediaAsset[],
) {
  return assets.find(
    ...
    kind === "file"
      ...
      : kind === "model"
        ...
        : control.multiple === true && items.length > 1
          ? "image-grid"
          : "single-image";
  const persistenceStatus = kind === "model"
    ? createModelPersistenceStatus(assets)
    : undefined;
  const feedback = operation.feedback ??
    (kind === "model" ? getModelPersistenceFeedback(assets) : undefined);

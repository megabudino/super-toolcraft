...
test("test support routes only reverse-reachable executable unit and catalog proof", () => {
  ...
  assert.deepEqual(result.performancePassIds, []);
  assert.deepEqual(result.performanceTestNames, []);
  ...
  assert.deepEqual(mixed.performancePassIds, ["preview-composite"]);
  assert.deepEqual(mixed.performancePathIds, [catalog.performance[0].pathId]);
  assert.deepEqual(mixed.performanceTestNames, [catalog.performance[0].testName]);
  assert.equal(mixed.kind, "performance");
});
...
test("performance specs include only real reverse-reachable Vitest dependents", () => {
  ...
  assert.deepEqual(result.productTestFiles, [unitPath]);
  assert.deepEqual(result.performancePassIds, ["preview-composite"]);

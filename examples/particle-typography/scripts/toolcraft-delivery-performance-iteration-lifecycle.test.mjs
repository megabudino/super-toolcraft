  ...
  const planningInputs = Object.freeze({
    ...
    changeSet: Object.freeze({
      ...
      impact: Object.freeze({
        ...
        kind: "performance",
        performancePassIds: Object.freeze(["composite"]),
        performancePathIds: Object.freeze([pathId]),
        performanceTestNames: Object.freeze([performanceTestName]),
...
test("performance authority remains consumed across an ordinary delivery", () => {
  const authorityHash = hash("d");
  const first = createFixture({ authorityHash });
  ...
  const ordinaryCurrent = inventory("3");
  const ordinaryInputs = {
    ...
    currentInventory: ordinaryCurrent,
    integrity: {
      manifestHash: hash("a"),
      sourceHash: ordinaryCurrent.sourceHash,
    },
    previousPerformance: firstAnchor.performance,
    previousLifecycle: firstAnchor.lifecycle,
  };
  const ordinaryPlan = createToolcraftDeliveryPlan(ordinaryInputs);
  const ordinaryReceipt = createToolcraftDeliveryReceipt({
    plan: ordinaryPlan,
    result: createResult(ordinaryPlan, ordinaryCurrent),
  });
  const ordinaryAnchor = normalizeToolcraftDeliveryAnchor(ordinaryReceipt);
  ...
  assert.throws(
    () => createToolcraftDeliveryPlan({
      ...first.planningInputs,
      comparisonInventory: {
        entries: ordinaryAnchor.files,
        sourceHash: ordinaryAnchor.sourceHash,
      },
      ...
      previousLifecycle: ordinaryAnchor.lifecycle,
      previousPerformance: ordinaryAnchor.performance,
    }),
    /authority has already been used/iu,
  );
});

test("performance planning requires a previous successful changed delivery", async () => {
  const fixture = createFixture();
  ...
  await assert.rejects(
    executeToolcraftDeliveryLifecycleCore({
      ...
      projectDir: "/tmp/toolcraft-missing-performance-anchor",
    }),
    /prototype planning inputs contain comparison-only state/iu,
  );
});

...
import * as legacyTargetedEvidence from "./toolcraft-legacy-targeted-evidence.mjs";
...
test("delivery compatibility is isolated to one anchor boundary", async () => {
  assert.deepEqual(Object.keys(deliveryAnchor).sort(), [
    "TOOLCRAFT_LEGACY_DELIVERY_RECEIPT_VERSION",
    "TOOLCRAFT_LEGACY_FULL_PERFORMANCE_DELIVERY_CHECKS",
    "TOOLCRAFT_LEGACY_PROTOTYPE_DELIVERY_CHECKS",
    "assertToolcraftLegacyDeliveryBaselineCoherence",
    "getToolcraftDeliveryAnchorShapeError",
    "getToolcraftDeliveryCompatibility",
    "isToolcraftLegacyTargetedDeliveryMode",
  ...
  const productionScripts = (await fs.readdir(import.meta.dirname))
    .filter((name) => name.endsWith(".mjs") &&
      !name.endsWith(".test.mjs") &&
      !name.endsWith("-test-helpers.mjs") &&
      name !== "toolcraft-delivery-anchor.mjs");
  const duplicateLegacyVersionSets = [];
  for (const scriptName of productionScripts) {
    const source = await fs.readFile(path.join(import.meta.dirname, scriptName), "utf8");
    if (/\[\s*2\s*,\s*3\s*,\s*4\s*\]/u.test(source)) {
      duplicateLegacyVersionSets.push(scriptName);
    }
  }
  assert.deepEqual(duplicateLegacyVersionSets, []);
});
...
test("current and legacy evidence remain separate receipt boundaries", () => {
  assert.deepEqual(
    Object.keys(performanceReceiptPolicy).sort(),
    expectedPerformanceReceiptPolicyExports,
  );
  assert.deepEqual(Object.keys(legacyTargetedEvidence).sort(), [

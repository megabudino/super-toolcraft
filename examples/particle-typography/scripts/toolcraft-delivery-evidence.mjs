...
const evidenceKeys = Object.freeze({
  ...
  "browser-functional-smoke": ["kind", "smokeEvidence", "testNames"],
  "browser-performance": [
    "kind",
    "passIds",
    "pathIds",
...
const smokeEvidenceKeys = Object.freeze([
  "fixtureSelector",
  "kind",
  "sourceHash",
  "testName",
...
function getSmokeEvidenceError(step, evidence, plan) {
  const smoke = evidence.smokeEvidence;
  return !hasExactKeys(smoke, smokeEvidenceKeys) ||
    smoke.fixtureSelector !== "development" ||
    smoke.kind !== "performance-smoke" ||
    smoke.sourceHash !== plan.sourceHash ||
    smoke.testName !== step.smokeTestName
    ? "Toolcraft combined browser smoke evidence does not match its plan."
    : undefined;
}
...
function getPerformanceEvidenceError(step, evidence, plan) {
  ...
  return reportHash === evidence.reportHash
    ? undefined
    : "Toolcraft targeted performance report hash does not match its evidence.";
}
...
export function getToolcraftProofStepEvidenceError(step, evidence, plan) {
  ...
  if (
    ["browser-functional", "browser-functional-smoke"].includes(step.kind) &&
    !arraysEqual(evidence.testNames, step.testNames)
  ) {
    return "Toolcraft browser proof step evidence does not match its plan.";
  }
  if (step.kind === "browser-functional-smoke") {
    return getSmokeEvidenceError(step, evidence, plan);
  }

...
function hasSelectedProof(impact) {
  return [
    ...
    impact.performancePathIds,
    impact.performanceTestNames,
    impact.productTestFiles,
  ].some((values) => values.length > 0);
}

function normalizePreviousPerformance(performance) {
  if (
    performance.kind !== "none" &&
    performance.report.version !== 3
  ) {
...
export async function loadToolcraftDeliveryPlanningInputs({
  ...
  return Object.freeze({
    ...
    previousPerformance: normalizePreviousPerformance(
      previous.anchor.performance,
    ),
  });
}

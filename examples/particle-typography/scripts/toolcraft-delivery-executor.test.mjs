...
function createNoopOperations(overrides = {}) {
  return freeze({
    ...
    runBrowserFunctionalSmoke: async () => {},
...
test("a combined prototype step invokes browser proof once", async () => {
  const { plan, result: expected } = createPlanReceiptFixture("prototype");
  const invocations = [];
  const operations = createNoopOperations({
    collectInventory: async () => expected.finalInventory,
    runBrowserFunctionalSmoke: async (input) => {
  ...
  assert.deepEqual(invocations[0].testNames, [
    ...plan.steps.at(-1).testNames,
    plan.steps.at(-1).smokeTestName,
  ]);
...
test("same-process authority reserves one exact receipt", async () => {
  ...
  const receipt = createToolcraftDeliveryReceipt({ plan, result: execution });

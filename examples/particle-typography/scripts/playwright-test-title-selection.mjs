...
function exactKeys(value, expected, label, errors) {
  ...
  const unknown = actual.filter((key) => !expectedSet.has(key)).sort(compareCodeUnits);
...
function normalizeCatalogAcceptance(value, index, errors) {
  ...
  exactKeys(value, ["acceptanceId", "file", "testName"], label, errors);
  for (const key of ["acceptanceId", "file", "testName"]) {
  ...
  if (![value.acceptanceId, value.file, value.testName].every(isTrimmedString)) {
    return undefined;
  }
...
export function validateToolcraftDeliveryCatalog(value) {
  ...
  if (value.version !== 1) errors.push("Delivery catalog.version must be 1.");

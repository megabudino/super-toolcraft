...
function createPrototypeReceipt(inventory) {
  const plan = {
    kind: "prototype",
...
test("current impact validation accepts an owned public resource without requiring every resource", async (t) => {
...
test("verify receipt accepts current baseline-free delivery authority", async (t) => {
  ...
  t.after(() => fs.rm(rootDir, { force: true, recursive: true }));
  const inventory = await collectToolcraftVerificationInputs(rootDir);
  await writeDeliveryReceiptFixture(rootDir, createPrototypeReceipt(inventory));
...
test("verify receipt rejects legacy delivery as current authority", async (t) => {
  ...
  await writeDeliveryReceiptFixture(rootDir, {
    checks: ["integrity", "ai-check", "test", "build", "playwright-functional", "playwright-smoke"],

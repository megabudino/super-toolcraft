...
| Tier 2 — schema/product behavior              | Controls, sections, defaults, persistence, panel actions, export actions, acceptance rows, or product behavior mapping changes.                                                                                                                                                                | During development run relevant targeted tests and browser acceptance. At delivery, bare `npm run verify:delivery` derives the required proof.                                                                                                                                                                                                                             |
...
The automatic lifecycle is sequential. First product delivery uses bare `npm run verify:delivery` for complete product contracts, one production build, and one combined functional/prototype-smoke browser run. Ordinary later delivery uses the same bare command and derives exact proof from current ownership plus the immediately previous delivery. A performance complaint records only domain authority in the worklog—an exact request quote and canonical affected path IDs—then one bare delivery runs one targeted iteration and returns the app for evaluation. A full audit is separate: only an explicit operator request or accepted offer authorizes `npm run verify:perf`, which performs one fresh build and the complete performance matrix.

Feature loops after the first working version run only the smallest affected checks while implementation is changing. At the delivery boundary, `verify:delivery` compares against the immediately previous successful delivery and runs exact affected checks once. Filename, verification tier, and touched subsystem do not trigger the full suite by themselves. Record human decisions and risks in the worklog; protected receipts own the executed plan and evidence.

The app is not complete until bare `npm run verify:delivery` produces the lifecycle-appropriate protected receipt. A prototype receipt proves full functionality and bounded responsiveness, not full performance certification. Agent-controlled browser checks remain useful for targeted diagnosis and visual verification, but terminal prose cannot mint or upgrade a receipt. The impact inventory derives the required checks and affected passes from the immediately previous successful delivery. Tier is human diagnostic vocabulary only and never grants execution authority.

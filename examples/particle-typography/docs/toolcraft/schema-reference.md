...
Explicit `include` augments rather than removes capability-derived slices. `{ storage: "none" }` is the only opt-out; record why losing the workspace on reload is intentional.

...
Use focused non-durable checks during edits, then one automatic delivery: first product delivery uses bare `npm run verify:delivery` for complete contracts, one build, and one functional/prototype-smoke browser run; ordinary later delivery uses it for ownership-derived proof; a performance complaint adds request/path authority and gets one targeted delivery; a full audit needs explicit operator authority for `npm run verify:perf`, one build, and the complete matrix. `app-verification-impact.json` drives scope. Evidence levels and classification live in `core/performance.md` and `performance.md`; prose cannot mint evidence.
...
Default local persistence requires one runtime row with `evidence: "persistence-state"`, `persistenceCoverage: "reload"`, and `persistenceSlices` equal to the resolved plan. Browser proof changes user state for each available slice, waits for status `success`, reloads, and verifies restored UI or output. Settings import/export is not reload proof.

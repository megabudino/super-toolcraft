...
When a selector controls visibility or enabled state for another control through `visibleWhen` or `disabledWhen`, treat both as one dependency group if they share the same target entity. Keep the selector and its gated branch controls in the same section; do not create separate `Image`, `Texture Image`, `Text Style`, or similar branch sections unless the branch is a genuinely separate product entity with its own workflow evidence.

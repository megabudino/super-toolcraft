...
If `visibleWhen` or `disabledWhen` points to a mode/type/source selector for the same target entity, keep the selector and the dependent controls in the same semantic section. Do not split `Source` from its image upload, `Texture` from `Texture Image`, or `Text` from its content/style only because the dependent branch uses a standalone control.
...
Double-clicking the vector pad resets both axes to `0,0` through the normal runtime value update. Do not add a separate custom reset button for basic pad-centering behavior.
...
If only `Export Settings` and `Import Settings` appear in that section, the schema is not using `editable-output` canvas sizing or already owns `canvas.size.width` / `canvas.size.height` controls. For product-output apps, prefer fixing the canvas sizing decision over adding hand-built size fields. A reference or previous app lacking a size editor does not justify `fixed-output`; use `fixed-output` only for a real fixed-format or explicitly locked dimension requirement.

...
export async function getToolcraftFieldByLabel(page: Page, label: string): Promise<Locator> {
  const exactLabel = page
    .locator('[data-slot="template-field-label-text"]')
    .filter({ hasText: new RegExp(`^${escapeRegExp(label)}$`) });
  const exactField = page.locator('[data-slot="field"]').filter({ has: exactLabel }).first();

  if ((await exactField.count()) > 0) {
    await expect(exactField, `Toolcraft field "${label}" should be visible`).toBeVisible();
    await exactField.scrollIntoViewIfNeeded();
    return exactField;
  }

  const field = page
    .locator('[data-slot="field"]')
    .filter({ hasText: new RegExp(`^${escapeRegExp(label)}(?!\\s+[a-z])`) });

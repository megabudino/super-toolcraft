...
export async function getToolcraftProductObservableSnapshot(
  ...
  return observable.evaluate((element, requestedCanvasSampleSize) => {
    ...
    const snapshotCanvas = (canvas: HTMLCanvasElement) => {
      ...
      const sampleSize = Math.max(1, Math.min(512, requestedCanvasSampleSize));
      const sampleWidth = Math.max(1, Math.min(sampleSize, width));
      const sampleHeight = Math.max(1, Math.min(sampleSize, height));

...
test("browser perf: initial render collects enough frames for a real p95", () => {
  expect(
...
test("browser perf: render-scale paths require real backing-pixel proof configuration", () => {
  const renderScalePath = {
    id: "performance-path:render-scale",
    interaction: "control-change",
    invalidates: ["preview"],
    profile: "interactive-discrete",
    runsOn: ["main"],
  ...
  expect(() =>
    compileToolcraftPerformancePathAdapterMatrix(
      [renderScalePath],
      [adapter(renderScalePath.id)],
    ),
  ).toThrow(/render-scale.*backing-pixel proof/iu);
  ...
  ).toThrow(/render-scale backing-pixel proof.*non-null object/iu);

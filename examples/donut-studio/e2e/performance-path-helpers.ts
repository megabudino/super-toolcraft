...
import {
  assertToolcraftRenderScaleBackingProofConfig,
  expectToolcraftPerformanceRenderScaleBackingEvidence,
  isToolcraftRenderScalePerformancePath,
...
export function compileToolcraftPerformancePathAdapterMatrix(
  ...
  for (const path of canonicalPaths) {
    ...
    if (
      isToolcraftRenderScalePerformancePath(path) &&
      !adapter.renderScaleBacking
    ) {
      throw new Error(
        `Toolcraft render-scale performance path "${path.id}" requires real backing-pixel proof configuration.`,
      );
    }
    if (adapter.renderScaleBacking) {
      assertToolcraftRenderScaleBackingProofConfig(adapter.renderScaleBacking);
    }

...
| `external-site` | Host-only external document, dedicated Settings profile, local responsive viewport preferences and transient authenticated geometry; conflicts with graphical renderers, timeline and artifact export |
...
External-site contributes `external-document` sizing through the canvas-behavior pipeline. Its Setup uses canonical workspace-color and width/height controls, with a 1920px default width and measured content-height cap. A module-owned Page select is the final Settings control. Its explicit catalog comes from the document port and contains one representative per unique source-inspected page composition, not every content URL. Native navigation never appends choices; confirmed guest locations persist as local view preferences, never shared CSS values or source defaults. The navigation port handles full-document and SPA transitions without replacing the iframe or requiring a framework router dependency. Width changes restore automatic full-height fitting. View preferences persist per window; measurements are transient and neither enter site history nor write source defaults. A session port supplies geometry through the development SDK; the runtime does not depend on that npm package. Generated graphical products cannot select this host capability to bypass their image-export proof contract.

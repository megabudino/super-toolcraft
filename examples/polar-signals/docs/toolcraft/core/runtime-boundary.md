...
- SVG presentation roots and transport wrappers allow overflow in both modes; the runtime artboard is the finite presentation clip. Library-created SVG viewports must not retain browser-default clipping. Internal semantic masks/clips remain product-owned. Follow the edge-pixel proof in `core/setup-export.md#infinity-canvas`; changing outer runtime CSS cannot recover already clipped SVG or raster output.

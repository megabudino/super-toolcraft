...
| Connected website draft, shared history and source-save recovery intent | Host workspace `.toolcraft/workspaces/projects/<canonical-source-hash>/` | Durable Project Bridge state. It belongs to the Toolcraft host, never the website, browser diagnostics or generic cleanup. The user-local process ownership lock contains metadata only. |
| Connected website registrations and active startup selection | Host `.toolcraft/projects/<project-id>.json` and `.toolcraft/projects/.active.json` | Durable host configuration, written by connect/activate. Never treat it as disposable scratch or copy it into the website. |

...
The website adapter uses the same runtime Import/Export Settings actions and app-specific JSON envelope. Imported parameter values dispatch one `settings.apply` command to the shared website authority as a bulk `setMany`; the normal shared acknowledgement/history flow updates the iframe and other editors. Import checks admission again after file/resource reads. Disconnected or read-only shared sessions disable file/default actions.

Local Save State as Default uses the source authoring capability: it waits for accepted shared values, waits for the current website document to confirm those values, then writes the website's source parameters through the existing host. A replaced/disconnected document aborts the save. The adapter does not write a runtime `app-defaults.json` or treat editor viewport, page navigation, toolbar preferences, graphics layers or timeline state as website CSS defaults. Production exposes no source authoring capability.
...
- Product SVG roots and outer wrappers allow overflow in both modes (`overflow: visible`, no paint containment); libraries may retain default SVG clipping. Preserve viewBox and geometry. Internal semantic masks/nested SVG viewports remain valid. The mode observer checks every outer SVG, including unmarked roots beside raster output. Prove real pixels beyond each affected artboard edge in Infinity and their absence in finite mode. See `core/runtime-boundary.md#canvas-boundary` for renderer ownership.
...
- PNG/JPEG/SVG export always uses the saved centered finite artboard, including in Infinity mode. Content outside that rectangle is clipped; pan, zoom and workspace mode never change the export. Hidden or unavailable layers, runtime media suppressed by the composition, and editor-only handles or gizmos remain excluded. SVG requires real vector content; runtime media/model pixels are not traced or wrapped in SVG.
...
- Product renderers receive the canonical provider rect used by the live scene, while artifact output uses the saved centered artboard. The product world frame and the export crop have separate responsibilities in both modes.
...
Infinity artifact proof uses the existing `scene-bounds-image-export`, `scene-bounds-svg-export` and `scene-bounds-video-export` coverage keys. The helpers accept one `expectedSize` for both modes and compare decoded image pixels, SVG viewBox/content, or video duration/frame count/decoded samples. Test matching scene/artboard bounds, smaller and larger artboards, and shifted content; equal dimensions alone cannot prove correct cropping. The unavailable-image helper proves resource exclusion only and never substitutes for finite/Infinity parity.
...
- The saved finite artboard crop, selected resolution and 30 FPS video cadence apply in both modes. Video evaluates immutable frame states on demand without a bounds prepass or retaining complete state copies.

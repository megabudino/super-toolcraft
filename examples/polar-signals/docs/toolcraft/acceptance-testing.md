...
`infinityCanvasCoverage: "mode-continuity-and-restoration"` proves `infinity-mode-continuity`: only boundary, clipping and size controls change. The observer checks outer SVG/wrapper clipping; product tests prove edge pixels and finite restoration. `scene-bounds-{image,svg,video}-export` proves identical saved-artboard crop, dimensions, background and decoded content. See [full invariants and proof](core/setup-export.md#infinity-canvas).

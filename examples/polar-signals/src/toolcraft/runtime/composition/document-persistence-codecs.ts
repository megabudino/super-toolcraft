import {
  documentViewportTargets,
} from "../modules/built-ins/external-site/viewport-schema";
...
function pickPersistedValues(
  ...
  for (const target of targets) {
    if (
      state.documentViewport &&
      Object.values(documentViewportTargets).some((value) => value === target)
    )
      continue;

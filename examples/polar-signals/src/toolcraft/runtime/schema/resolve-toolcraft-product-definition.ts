import { resolveExternalDocumentCanvas } from "../modules/built-ins/external-site/viewport-schema";
...
export function resolveToolcraftProductDefinition(
  ...
  const materializedSchema = {
    canvas: resolveExternalDocumentCanvas(
      base.canvas,
      resolution.contributionResolution.canvasBehaviors,
    ),
  ...
  const schema: ResolvedToolcraftAppSchema = freezeRecursively({
    ...resolveToolcraftMaterializedSchema({
      modulePanels: resolution.contributionResolution,
      identity,

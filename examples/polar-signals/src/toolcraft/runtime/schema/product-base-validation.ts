...
export function getToolcraftProductBaseSemanticValidationErrors(
  ...
  if (
    base.canvas.sizing?.mode === "external-document" &&
    !contributions.canvasBehaviors.some((item) => item.behavior === "external-document")
  )
    errors.push("External document sizing requires externalSiteModule().");
  if (

import { initializeDocumentViewport } from "../modules/built-ins/external-site/viewport-state";
...
export function createToolcraftState(
  ...
  return initializeDocumentViewport({
    ...
    mediaAssets,
    panels: {
      ...(schema.assembly.surfaces.panels["document-toolbar"]?.enabled ? {
        "document-toolbar": { offset: { x: 0, y: 0 }, ...initialState.panels?.["document-toolbar"] },
      } : {}),
      controls: {
        ...panels.controls,

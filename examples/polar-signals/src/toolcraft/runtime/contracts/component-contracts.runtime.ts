...
const runtimeSetupModeRowRule =
  "Runtime Setup places the standard Background switch beside Infinity canvas inside Settings, below the settings file/defaults action section, then Background color beside the Blanc/Dots workspace selector and finite sizing; Timeline and optional Lock rotation share the final Setup row when enabled.";
...
export const TOOLCRAFT_RUNTIME_COMPONENT_CONTRACTS = {
  ...
  settingsTransfer: {
    aiUsageRules: [
      ...
      "Import Settings and Export Settings share the headerless block above Settings in every host. Local Save State as Default appears above them only with source authoring capability; preview and production hide only Save. Import and save serialize by runtime store across panel remounts. Settings uses standard section Reset and collapse controls; the panel-header Reset remains available in every host.",
  ...
  controlLabels: {
    aiUsageRules: [
      ...
      "The settings file/defaults action block uses public technical spacing with 12px on all sides. Settings uses the standard section header and ordinary body spacing, with 8px top and 24px bottom insets; sticky footer action sections keep their dedicated spacing.",

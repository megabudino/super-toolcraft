...
import { applyToolcraftSharedControlDefault, useToolcraftSharedControlDefault, useToolcraftSharedControlsDisabled } from "../../external-document/shared-values-context";
...
export const ControlsPanelControlGroup = React.memo(
  function ControlsPanelControlGroup({
    ...
    const sharedDisabled = useToolcraftSharedControlsDisabled(entries.map(([, control]) => control.target));
    const sharedDefault = useToolcraftSharedControlDefault(entries[0]?.[1].target);
    ...
    const rawControl = applyToolcraftSharedControlDefault(declaredControl, sharedDefault);
    const disabled = sharedDisabled || selection.disabledById[id] === true;
    ...
    switch (rendererKind) {
      ...
      case "basic":
        node = renderBasicControl({
          commit,
          control,
          id: sharedDisabled ? `${id}:synchronizing` : id,
          name,

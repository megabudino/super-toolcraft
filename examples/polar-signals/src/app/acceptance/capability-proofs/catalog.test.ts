...
const expectedRecipes = {
  ...
  "document.external-site": {ownerId:"external-site",proof:{kind:"external-site"}},
...
describe("Toolcraft capability proof catalog", () => {
  ...
  it.each(allCapabilities.filter(capability=>capability!=="document.external-site"))(
...
it("refuses generated-product certification of a host-owned website",()=>{expect(validateToolcraftCapabilityProofs({context:createCapabilityProofValidationContextFixture(),plan:createResolvedProductModulePlanFixture(["document.external-site"])}).join(" ")).toContain("project-host website verification workflow");});

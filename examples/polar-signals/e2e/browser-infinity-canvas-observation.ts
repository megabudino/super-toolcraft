...
export async function observeInfinityCanvas(
  ...
  return page.evaluate(() => {
    ...
    // Observe every outer SVG, including library-owned roots and SVG beside a
    // raster surface. Internal SVG viewports/masks belong to product geometry.
    const presentationNodes = new Set<Element>();
    for (const svg of productScene?.querySelectorAll("svg") ?? []) {
      if (svg.parentElement?.closest("svg")) continue;
      const bounds = svg.getBoundingClientRect();
      if (bounds.width === 0 || bounds.height === 0) continue;
      for (let node: Element | null = svg; node && node !== surface; node = node.parentElement) {
    ...
    for (const node of presentationNodes) {
      ...
      if (/\b(paint|strict|content)\b/u.test(style.contain)) {
...
export function expectInfiniteCanvasObservation(
  ...
  expect(observation.productClipping, "Infinity product presentation must not clip its SVG roots or transport wrappers; the runtime artboard owns finite clipping.").toEqual([]);

...
export async function configureRendererProvider({
  ...
  if (!result.changed) {
    return Object.freeze({ changed: false, installCommand: "pnpm install" });
  }
  ...
  return Object.freeze({ changed: true, installCommand: "pnpm install" });
}
...
async function runCli() {
  ...
  if (command !== "enable" || providerId !== "vgpu" || remaining.length > 0) {
    throw new TypeError("Usage: pnpm toolcraft:renderer -- enable vgpu");
  }

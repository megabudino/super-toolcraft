...
export async function getToolcraftFieldByLabel(page: Page, label: string): Promise<Locator> {
  ...
  const labeledField = page.locator('[data-slot="field"]').filter({ has: exactLabel });
  const field =
    (await labeledField.count()) > 0
      ? labeledField
      : page.locator('[data-slot="field"]').filter({
...
export async function dragToolcraftSliderByLabel(
  ...
  await expect(track, `Toolcraft slider "${label}" track should be visible`).toBeVisible();
  ...
  const endX = trackBox.x + trackBox.width * targetRatio;
...
export async function dragToolcraftCanvasViewport(
  ...
  startRatio: { x: number; y: number } = { x: 0.12, y: 0.1 },
): Promise<void> {
  ...
  const startX = box.x + box.width * startRatio.x;
  const startY = box.y + box.height * startRatio.y;

...
export const appTransferMode: ToolcraftTransferMode = {
  animationIntent: {
    loopDuration: {
      evidence:
        "An eight-second product-derived cycle gives every integer-speed motion type enough time to read clearly while preserving a seamless forward loop.",
      seconds: 8,
...
export const appProductReadiness: ToolcraftProductReadiness = {
  ...
  productSummary:
    "A procedural WebGL motion artwork containing one configurable dot volume with four point layouts, seven color-distribution modes, curated palette presets, dot shape and glow styling, editable radial bounds, expanded and signed Z curvature, helical depth, perspective, 3D orientation, point inertia, seven seamless motion families, sparkle, one-click randomize, and finite or infinite canvas editing.",
  requestedBehavior:
    "Make the artwork visually varied and configurable: restructure the dot field into rings, sunflower, spiral arms, or arcs; recolor it through palette presets, distribution modes, highlight, and accent density; restyle points with shapes, size profile, and neon glow; extend motion with flow, sweep, pulse, and sparkle; and add curated randomize actions while preserving 3D volume, persistence, and PNG/video export.",
};

const controlAcceptance = (
  id: string,
  target: string,
  componentType: string,
  userAction: string,
  ...
  automated: true,
  automatedTestName: `${target} maps to deterministic mosaic output`,
  browser: true,
  browserTestName: "browser: kinetic circle controls change rendered output",
  componentType,
  evidence: "rendered-pixels",
  expectedObservable,
  fixture: "Default one-circle 1920×1080 mosaic with the playback timeline paused at a deterministic frame.",
  id,
  kind: "control",
  target,
  userAction,
...
export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  controlAcceptance(
    ...
    "canvas.renderScale",
    "slider",
    "Drag the discrete Resolution scale slider between 1× and 2× while keeping the logical canvas size unchanged.",
    "The WebGL backing canvas pixel width and height change with the selected scale while CSS/output dimensions remain 1920×1080.",
    { browserTestName: "browser: kinetic circle resolution scale is discrete and smooth" },
  ),
  controlAcceptance(
    "canvas-infinity",
    "canvas.infinity",
    "switch",
    "Enable Infinity canvas in Setup, pan and zoom the unbounded scene, then disable it.",
    "Finite artboard bounds and size controls disappear without changing the dormant output size; disabling Infinity restores the exact finite frame centered in the viewport.",
    {
      automatedTestName:
        "canvas.infinity maps to finite and infinite runtime canvas modes",
      browserTestName:
        "browser: Infinity canvas preserves finite size, history, and persistence",
      evidence: "viewport-side-effect",
    },
  ),
  {
    actionCoverage: ["randomize-look", "randomize-colors"],
    automated: true,
    automatedTestName:
      "panel.variations maps curated random values into runtime state",
    browser: true,
    browserTestName: "browser: kinetic circle variations randomize the look",
    componentType: "actions",
    evidence: "rendered-pixels",
    expectedObservable:
      "Randomize look changes structural, motion, and palette targets plus rendered pixels through undoable history entries ending with the palette preset; Randomize colors changes only palette-related targets and visibly recolors the field.",
    fixture:
      "Default mosaic with playback paused; click each Variations button, inspect runtime values and canvas pixels, then use Undo.",
    id: "variations-randomize",
    kind: "control",
    target: "panel.variations",
    userAction:
      "Click Randomize look and Randomize colors in the Variations section, then use Undo to restore the previous look.",
  },
  controlAcceptance(
    "shape-form",
    "shape.form",
    "select",
    "Choose Circle, Flower, Pinch, and Vortex from the visible Form menu.",
    "Each Form option produces a distinct single-object silhouette while the object remains centered in the same canvas.",
    {
      browserTestName: "browser: kinetic circle shape forms change one object",
      optionCoverage: "each-visible-item",
    },
  ),
  ...[
    ["shape-bend", "shape.bend", "Bend", "contour deformation amplitude"],
    ["shape-depth", "shape.depth", "Depth", "center-to-edge deformation profile"],
    ["shape-rotation", "shape.rotation", "Contour angle", "authored planar contour orientation"],
  ].map(([id, target, label, result]) =>
    controlAcceptance(
      id,
      target,
      "slider",
      `Drag ${label} while the timeline is paused.`,
      `The single circle's ${result} changes live without changing canvas size or vertex count.`,
      {
        browserTestName:
          "browser: every kinetic circle slider has an expressive independent range",
      },
    ),
  ),
  {
    automated: true,
    automatedTestName:
      "view.orbit maps one validated 3D pose to preview and export",
    browser: true,
    browserTestName:
      "browser: orientation gizmo and direct object drag rotate one 3D relief",
    canvasHandle: {
      exportCleanTestName:
        "browser: exported mosaic follows orientation without editor gizmo pixels",
      outputObservable:
        "The dot relief foreshortens and exposes different depth contours while remaining one object.",
    ...
    evidence: "rendered-pixels",
    expectedObservable:
      "Dragging or snapping the starter gizmo and dragging visible object pixels update the same persisted view.orbit pose, visibly reproject the 3D relief, support reset/undo/redo, and leave background drag available for viewport pan.",
    fixture:
      "Default slightly tilted relief with playback paused; drag the gizmo, snap an axis, drag a bright object region, and drag the empty canvas background.",
    id: "shape-orientation",
    kind: "canvas-handle",
    target: "view.orbit",
    userAction:
      "Drag the 3D orientation gizmo, click an axis snap target, drag a visible dot region, then use Undo, Redo, and Reset.",
  },
  controlAcceptance(
    "volume-radius-range",
    "volume.radiusRange",
    "rangeSlider",
    "Drag the lower and upper Ring radius thumbs independently while playback is paused.",
    "The inner boundary opens or closes and the outer boundary expands or contracts while the object remains centered in the same canvas.",
    {
      browserTestName:
        "browser: every kinetic circle slider has an expressive independent range",
  ...
  ...[
    ["volume-z-spread", "volume.zSpread", "Z spread", "expanded front-to-back extent"],
    ["volume-z-bend", "volume.zBend", "Z bend", "signed concave-to-convex dome curvature"],
    ["volume-z-twist", "volume.zTwist", "Z twist", "signed helical depth contour"],
    ["volume-perspective", "volume.perspective", "Perspective", "depth foreshortening and point scale"],
  ].map(([id, target, label, result]) =>
    controlAcceptance(
      id,
      target,
      "slider",
      `Drag ${label} while the timeline is paused and the object is tilted.`,
      `The single circle's ${result} changes live without moving the canvas viewport.`,
      {
        browserTestName:
          "browser: every kinetic circle slider has an expressive independent range",
      },
  ...
  controlAcceptance(
    ...
    "shape.repeats",
    "slider",
    "Drag the discrete Repeats slider through marked values.",
    "The number of visible contour features changes while the renderer keeps one centered procedural object.",
    {
      browserTestName:
        "browser: kinetic circle discrete controls are smooth",
    },
  ),
  controlAcceptance(
    "pattern-density",
    "pattern.density",
    "slider",
    "Drag Density while the canvas is visible.",
    "The number of dot vertices changes live in the single form and the rendered pixel snapshot changes.",
    {
      browserTestName:
        "browser: every kinetic circle slider has an expressive independent range",
    },
  ...
  controlAcceptance(
    ...
    "pattern.dotSize",
    "slider",
    "Drag Dot size while the canvas is visible.",
    "Every dot grows through the WebGL point-size uniform without changing the canvas bounds.",
    {
      browserTestName:
        "browser: every kinetic circle slider has an expressive independent range",
    },
  ...
  controlAcceptance(
    ...
    "pattern.seed",
    "slider",
    "Drag the discrete Seed slider through marked values with the Toolcraft slider helper.",
    "The deterministic angular, radial, point-size, and bright/accent scatter change while the concentric one-object structure remains stable.",
    {
      browserTestName:
        "browser: kinetic circle discrete controls are smooth",
    },
  ...
  controlAcceptance(
    ...
    "pattern.layout",
    "select",
    "Choose Rings, Sunflower, Spiral arms, and Arcs from the visible Layout menu.",
    "Each layout restructures the same bounded point budget into a visibly different arrangement while the object stays centered, and the Arms and Arc coverage controls appear only for their matching layout branch.",
    {
      browserTestName: "browser: kinetic circle layouts restructure the dot field",
      optionCoverage: "each-visible-item",
    },
  ),
  controlAcceptance(
    ...
    "pattern.arms",
    "slider",
    "Switch Layout to Spiral arms, then drag the discrete Arms slider through marked values.",
    "The number of visible galaxy arms changes at a stable point budget; switching layouts away and back preserves the chosen arm count.",
    {
      browserTestName: "browser: kinetic circle layouts restructure the dot field",
      fixture:
        "Spiral arms layout branch of the default mosaic with playback paused; the control stays hidden for other layouts.",
    },
  ),
  controlAcceptance(
    "pattern-arc-fill",
    "pattern.arcFill",
    "slider",
    "Switch Layout to Arcs, then drag Arc coverage between sparse and nearly closed arcs.",
    "The dashed arc coverage and vertex count change live; segment count follows Repeats, and the control stays hidden for other layouts.",
    {
      browserTestName: "browser: kinetic circle layouts restructure the dot field",
      fixture:
        "Arcs layout branch of the default mosaic with playback paused; the control stays hidden for other layouts.",
    },
  ),
  controlAcceptance(
    "style-dot-shape",
    "style.dotShape",
    "select",
    "Choose Disc, Ring, Square, Diamond, and Streak from the visible Dot shape menu.",
    "Every sample renders with the selected point mask while geometry, vertex count, and layout remain unchanged.",
    {
      browserTestName: "browser: kinetic circle dot style changes point rendering",
      optionCoverage: "each-visible-item",
    },
  ),
  controlAcceptance(
    ...
    "style.taper",
    "slider",
    "Drag Taper from the enlarged-core extreme to the enlarged-rim extreme while playback is paused.",
    "Dot footprints regrade across the field through one vertex-shader uniform without rebuilding geometry.",
    {
      browserTestName:
        "browser: every kinetic circle slider has an expressive independent range",
    },
  ...
  controlAcceptance(
    ...
    "style.glow",
    "slider",
    "Drag Glow from zero to full while playback is paused.",
    "Additive neon halos appear around every dot and the live preview brightens without moving the canvas viewport.",
    {
      browserTestName:
        "browser: every kinetic circle slider has an expressive independent range",
    },
  ...
  controlAcceptance(
    ...
    "palette.preset",
    "select",
    "Choose every visible palette preset including Custom.",
    "Each curated preset recolors the field with its five-color palette, hides the custom color pickers, and switching back to Custom restores the editable colors with their previous values.",
    {
      browserTestName:
        "browser: kinetic circle color modes and presets recolor the field",
      optionCoverage: "each-visible-item",
    },
  ),
  ...[
    ["palette-base", "palette.base", "Base navy", "dark dot population"],
    ["palette-bright", "palette.bright", "Bright ice", "broken luminous annulus"],
    ["palette-cyan", "palette.cyan", "Cyan", "bridge bands"],
    ["palette-violet", "palette.violet", "Violet", "sparse cool accents"],
    ["palette-warm", "palette.warm", "Warm accent", "rare warm sparks"],
  ].map(([id, target, label, result]) =>
    controlAcceptance(
      id,
      target,
      "color",
      `Choose a visibly different ${label} color while Preset is Custom.`,
      `The ${result} changes in preview pixels and the exported frame consumes the same color; the control stays hidden for curated presets.`,
      {
        fixture:
          "Custom preset branch of the default mosaic with the playback timeline paused at a deterministic frame.",
      },
    ),
  ),
  controlAcceptance(
    ...
    "palette.colorMode",
    "select",
    "Choose Bands, Gradient, Sectors, Spiral, Depth, Patches, and Duotone from the visible Mode menu.",
    "Each mode distributes the same palette across the field in a visibly different pattern at an unchanged vertex count.",
    {
      browserTestName:
        "browser: kinetic circle color modes and presets recolor the field",
      optionCoverage: "each-visible-item",
  ...
  controlAcceptance(
    ...
    "palette.highlight",
    "slider",
    "Drag Highlight across its full range while playback is paused.",
    "The mode-aware bright emphasis moves through the field and the rendered pixels change live.",
    {
      browserTestName:
        "browser: every kinetic circle slider has an expressive independent range",
    },
  ...
  controlAcceptance(
    ...
    "palette.accents",
    "slider",
    "Drag Accents from zero to full while playback is paused.",
    "The scattered warm and violet spark population moves from absent to dense confetti at a fixed vertex count.",
    {
      browserTestName:
        "browser: every kinetic circle slider has an expressive independent range",
    },
  ...
  controlAcceptance(
    ...
    "motion.type",
    "select",
    "Choose Ripple, Breathe, Twist, Orbit, Flow, Sweep, and Pulse while playback is active.",
    "Each visible Motion type produces a distinct timeline-driven deformation of the same authored form with a seamless forward loop.",
    {
      browserTestName: "browser: kinetic circle motion types change animation",
      optionCoverage: "each-visible-item",
    },
  ),
  ...[
    ["motion-speed", "motion.speed", "Speed", "complete cycles per seamless loop"],
    ["motion-strength", "motion.strength", "Amount", "mode-aware displacement amplitude"],
    ["motion-wavelength", "motion.wavelength", "Frequency", "spatial repetition frequency"],
    ["motion-damping", "motion.damping", "Falloff", "edge attenuation"],
    ["motion-turbulence", "motion.turbulence", "Drift", "independent tangential travel"],
    ["motion-z-motion", "motion.zMotion", "Z motion", "independent axial displacement"],
    ["motion-core-opening", "motion.coreOpening", "Core opening", "inner-boundary opening force"],
    ["motion-ball-weight", "motion.ballWeight", "Ball weight", "per-point inertia, phase lag, gravity bias, and depth settling"],
    ["motion-sparkle", "motion.sparkle", "Sparkle", "deterministic per-dot twinkle of brightness and size"],
  ].map(([id, target, label, result]) =>
    controlAcceptance(
      id,
      target,
      "slider",
      `Drag ${label} during playback and at a paused deterministic timeline frame.`,
      `The rendered ${result} changes live without moving the canvas viewport.`,
      {
        browserTestName:
          id === "motion-speed"
            ? "browser: kinetic circle discrete controls are smooth"
  ...
  controlAcceptance(
    ...
    "switch",
    "Turn Include off, inspect the live preview, export PNG, then export video.",
    "The preview hides its product background, PNG pixels preserve transparent alpha, and video keeps the selected product background.",
  ),
  controlAcceptance(
    "background-color",
    "appearance.background",
    "color",
    "Choose a visibly different background color with Include enabled.",
    "The live WebGL clear color and included PNG/video background pixels change to the selected color.",
  ),
  controlAcceptance(
    "image-format",
    "export.image.format",
    "select",
    "Choose PNG and JPG, then use Export PNG.",
    "The downloaded image MIME type and extension match each visible format option.",
    {
      browserTestName: "browser: kinetic circle exports image and video",
  ...
  controlAcceptance(
    ...
    "select",
    "Choose 2K and 4K, export each image, and decode the blobs.",
    "The exported long edge changes to the selected 2048 or 4096 pixel size; the 8K option remains available.",
    {
      browserTestName: "browser: kinetic circle exports image and video",
      evidence: "exported-bytes",
      optionCoverage: "each-visible-item",
  ...
  controlAcceptance(
    ...
    "export.video.format",
    "select",
    "Choose MP4 and WebM before exporting a one-second edited timeline duration.",
    "The encoder uses the selected supported MIME/container or a safe WebM fallback and returns playable bytes.",
    {
      browserTestName: "browser: kinetic circle exports image and video",
      evidence: "exported-bytes",
      optionCoverage: "each-visible-item",
  ...
  controlAcceptance(
    ...
    "select",
    "Choose Current and 4K, export, and load video metadata.",
    "Current uses even canvas dimensions and 4K fits within 3840×2160 while preserving aspect ratio.",
    {
      browserTestName: "browser: kinetic circle exports image and video",
      evidence: "exported-bytes",
      optionCoverage: "each-visible-item",
  ...
  controlAcceptance(
    ...
    "panel.exports",
    "panelActions",
    "Click Export Video and Export PNG and observe determinate sticky-footer progress until each Promise settles.",
    "Export Video returns playable timeline-duration bytes with frame progress; Export PNG returns selected-format bytes and dimensions; the accent progress indicator hides after completion.",
    {
  ...
  {
    automated: true,
    automatedTestName: "timeline playback drives a seamless duration-aware mosaic loop",
    browser: true,
    browserTestName: "browser: kinetic circle timeline playback is seamless and duration aware",
    componentType: "timeline",
    evidence: "timeline-output",
    expectedObservable:
      "Play and Pause playback advance and freeze pixels; scrubbing changes the rendered frame; editing duration remaps one full cycle; motion advances in one direction; first and last frames stitch without a visible jump after changing duration; there is no mirror, no yoyo, no ping-pong, and no reverse fallback.",
    fixture: "Eight-second loop changed through the Edit timeline duration UI, sampled at first frame, midpoint, last frame minus epsilon, and wrapped first frame.",
    id: "timeline-playback",
    kind: "runtime",
    timelineCoverage: "playback",
    timelinePlaybackCoverage: "all-playback-behavior",
    userAction:
      "Use Play playback, Pause playback, scrub Playback position, Disable loop, Enable loop, edit timeline duration, and compare first and last frames after the change.",
  },
  {
    automated: true,
    automatedTestName: "local persistence restores mosaic values canvas panels and timeline",
    browser: true,
    browserTestName: "browser: kinetic circle restores settings after reload",
    componentType: "persistence",
    evidence: "persistence-state",
    expectedObservable:
      "After page.reload, the edited form, 3D orientation, Infinity mode, dormant finite canvas size, timeline duration, and panel preference are restored and still drive product output.",
    fixture: "Change real user-facing controls, wait for the localStorage write, then perform a real browser page reload.",
    id: "persistence-reload",
    kind: "runtime",
    persistenceCoverage: "reload",
    userAction: "Edit settings, wait for persistence, reload the browser page, and inspect restored controls and output.",
  },
];

export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "Whole-artwork variation shuffle",
    groupingReason: "The two curated randomize commands re-roll the whole look or only the coloring so users can explore varied results in one click.",
    targets: ["panel.variations"],
    title: "Variations",
  },
  {
    entity: "Single circle silhouette",
    groupingReason: "Form parameters define the one object's contour and relief, while the editor-only starter gizmo writes its shared 3D orientation.",
    targets: ["shape.form", "shape.bend", "shape.rotation", "shape.depth", "shape.repeats", "view.orbit"],
    title: "Shape",
  },
  {
    entity: "Authored radial and depth volume",
    groupingReason: "The radial span, expanded thickness, signed primary bend, helical depth wave, and perspective jointly define the object's spatial volume.",
    targets: ["volume.radiusRange", "volume.zSpread", "volume.zBend", "volume.zTwist", "volume.perspective"],
    title: "Volume",
  },
  {
    entity: "Point arrangement structure",
    groupingReason: "Layout mode, point count, coverage, deterministic variation, and the layout-specific arm and arc parameters jointly define how the single form's samples are arranged.",
    targets: ["pattern.layout", "pattern.density", "pattern.dotSize", "pattern.arms", "pattern.arcFill", "pattern.seed"],
    title: "Dot Field",
  },
  {
    entity: "Per-point rendering style",
    groupingReason: "Dot shape mask, radial size grading, and additive glow define how each sample is drawn without changing the arrangement.",
    targets: ["style.dotShape", "style.taper", "style.glow"],
    title: "Dot Style",
  },
  {
    entity: "Palette source colors",
    groupingReason: "The curated preset choice and the five editable custom colors define which colors the field can use.",
    targets: ["palette.preset", "palette.base", "palette.bright", "palette.cyan", "palette.violet", "palette.warm"],
    title: "Mosaic Palette",
  },
  {
    entity: "Color distribution pattern",
    groupingReason: "Distribution mode, mode-aware highlight position, and accent density define how the chosen palette spreads across the field.",
    splitReason: "Palette selection and distribution are separate product entities so each section stays cohesive and under seven controls.",
    targets: ["palette.colorMode", "palette.highlight", "palette.accents"],
    title: "Coloring",
  },
  {
    groupingReason: "Motion family, speed, planar amount, and frequency define the principal timeline trajectory.",
    splitReason: "The trajectory stays concise here while the separate Dynamics stage owns volumetric response and inertia.",
    targets: ["motion.type", "motion.speed", "motion.strength", "motion.wavelength"],
    title: "Motion",
    workflowStage: "Trajectory",
  },
  {
    groupingReason: "Falloff, drift, axial travel, inner opening, point inertia, and per-dot twinkle jointly define the trajectory's volumetric response.",
    splitReason: "These response parameters are separated from trajectory selection so both sections remain product-cohesive and under seven controls.",
    targets: ["motion.damping", "motion.turbulence", "motion.zMotion", "motion.coreOpening", "motion.ballWeight", "motion.sparkle"],
    title: "Dynamics",
    workflowStage: "Volumetric response",
  },
  {
    entity: "Output background",
    groupingReason: "Background inclusion and color jointly define live preview and export compositing.",
    targets: ["export.includeBackground", "appearance.background"],
    title: "Background",
  },
  {
    workflowStage: "Still delivery",
    groupingReason: "Image format and resolution jointly configure the exported still file.",
    targets: ["export.image.format", "export.image.resolution"],
    title: "Image Export",
  },
  {
    workflowStage: "Motion delivery",
    groupingReason: "Video format and resolution jointly configure the encoded timeline output.",
...
export function validateToolcraftAcceptanceCoverage(
  ...
  const controlAcceptance = new Map(
    acceptance
      .filter(
        (entry) =>
          entry.kind === "control" || entry.kind === "canvas-handle",
      )

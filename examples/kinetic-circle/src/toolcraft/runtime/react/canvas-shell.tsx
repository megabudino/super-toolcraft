...
function readCanvasBackgroundColor(state: ToolcraftState): string | undefined {
  const backgroundSection = state.schema.panels.controls?.sections.find(
    (section) => section.title?.trim().toLowerCase() === "background",
  );
  const backgroundControl = backgroundSection
    ? Object.values(backgroundSection.controls).find(
        (control) => control.type === "color",
      )
    : undefined;
  const value = backgroundControl
    ? (state.values[backgroundControl.target] ?? backgroundControl.defaultValue)
    : undefined;
...
export function CanvasShell({
  ...
  const { mode, offset, size, zoom } = state.canvas;
  const scale = zoom / 100;
  const infiniteBackgroundColor =
    mode === "infinite" && state.values["export.includeBackground"] !== false
      ? readCanvasBackgroundColor(state)
      : undefined;
  ...
  return (
    <div
      ...
      data-toolcraft-infinite-background-color={infiniteBackgroundColor}
      ...
      style={
        infiniteBackgroundColor
          ? { backgroundColor: infiniteBackgroundColor }
          : undefined
      }
    >
      <div
      ...
      >
        {renderEditableCanvas ? (
          <div
            className={cn(
              "relative z-10",
              mode === "finite" ? "overflow-hidden" : "overflow-visible",
            )}
            data-toolcraft-canvas-content=""
            data-toolcraft-canvas-mode={mode}
            data-toolcraft-editable-canvas=""
            data-toolcraft-infinite-scene={mode === "infinite" ? "" : undefined}
            style={
              mode === "finite"
                ? {
                    height: size.height,
          ...
          >
            ...
            {children ? (
              <div
                className={mode === "finite" ? "absolute inset-0 z-20" : "contents"}

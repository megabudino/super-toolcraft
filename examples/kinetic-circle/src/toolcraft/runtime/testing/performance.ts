...
const vectorLayerRendererStrategies = new Set<ToolcraftRendererStrategy>([
  "canvas-2d",
  "dom",
  "svg",
]);
...
function getRendererLayerErrors(technique: ToolcraftRendererTechnique): string[] {
  ...
  for (const layer of layers) {
    ...
    if (
      ...
      errors.push(
        `rendererTechnique layer "${layer.id}" is editing-handles and must use canvas-2d/dom/svg with exportMode "excluded".`,
      );
    }
  }

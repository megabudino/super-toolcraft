    ...
    surfaces: {
      canvas: {
        capabilities: canvas.enabled
          ? unique<ToolcraftAssemblyCapability>([
              ...(canvasEditableSize
                ? (["canvas.editableSize", "canvas.infinity"] as const)
                : []),
...
function mergeCanvasSizeControlsIntoSettingsTransferSection({
  ...
  infinityCanvasControl: ToolcraftControlSchema;
...
function normalizePanels({
  ...
  const finiteCanvasCondition = {
    equals: false,
    target: canvasInfinityTarget,
  } as const;
  const infinityCanvasControl: ToolcraftControlSchema = {
    defaultValue: false,

...
const canvasInfinityTarget = "canvas.infinity";
const canvasSizeWidthTarget = "canvas.size.width";
const canvasSizeHeightTarget = "canvas.size.height";
const includeBackgroundTarget = "export.includeBackground";
...
> {
  const nextCanvas =
    ...
    "canvas.mode" in patch ||
    "canvas.offset" in patch
      ? {
          ...state.canvas,
          ...("canvas.mode" in patch
...
export function toolcraftReducer(
  ...
  switch (command.type) {
    case "controls.setValue": {
      ...
      if (isToolcraftCanvasInfinityTarget(command.target)) {
        if (typeof command.value !== "boolean") {
          return state;
        }

        const nextMode = command.value ? "infinite" : "finite";

        if (
          nextMode === "infinite" &&
          state.values[includeBackgroundTarget] === false
        ) {
          return state;
        }

        if (state.canvas.mode === nextMode) {
          return state;
        }

        return commitStatePatch(
          state,
          {
            after: {
              "canvas.mode": nextMode,
              ...(nextMode === "finite"
            ...
            label:
              command.label ??
              (nextMode === "infinite"
                ? "Enable Infinity canvas"
                : "Disable Infinity canvas"),
          },
          {
      ...
      if (
        command.target === includeBackgroundTarget &&
        command.value === false &&
    ...
    case "controls.reset": {
      ...
      const resetCanvasMode = state.canvas.mode !== "finite";

      if (resetCanvasSize || resetMediaPatch || resetCanvasMode) {
        return commitStatePatch(state, {
          after: {
            ...state.defaults,
            ...(resetCanvasMode
    ...
    case "controls.resetTargets": {
      ...
      const shouldResetCanvasMode =
        targetSet.has(canvasInfinityTarget) && state.canvas.mode !== "finite";
